...
#include "dicom.h"
#include "UnsethingQueryRetrieve/dcmqropt.h"
#include "dimse.h"
#include "UnsethingQueryRetrieve/dcmqrcnf.h"
#include "UnsethingQueryRetrieve/dcmqrsrv.h"
#include "dcdict.h"
#include "dcdebug.h"
#include "cmdlnarg.h"
#include "ofconapp.h"
...
#include "UnsethingQueryRetrieve/dcmqrdbq.h"
//#else
//#include "UnsethingQueryRetrieve/dcmqrdbi.h"
//#endif
...
UnsethingQueryRetrieveSCP *scp = nil;
UnsethingQueryRetrieveSCP *scptls = nil;

OFCondition mainStoreSCP(T_ASC_Association * assoc, T_DIMSE_C_StoreRQ * request, T_ASC_PresentationContextID presId, UnsethingQueryRetrieveDatabaseHandle *dbHandle)
{
...
- (void)run
{
    ...
    UnsethingQueryRetrieveOptions options;
...
//	opt_port = _port;
	...
	if([[_params objectForKey:@"TLSEnabled"] boolValue])
	{
		...
		if(certVerification==VerifyPeerCertificate || certVerification==RequirePeerCertificate)
		{
			...
			{
				if (EC_Normal != tLayer->addTrustedCertificateFile([[trustedCertificatesDir stringByAppendingPathComponent:cert] cStringUsingEncoding:NSUTF8StringEncoding], DCF_Filetype_PEM))
				{
...
UnsethingQueryRetrieveConfig config;
...
//    UnsethingQueryRetrieveIndexDatabaseHandleFactory factory(&config);
//#endif
	 //use if static scp rather than pointer
    //UnsethingQueryRetrieveSCP scp(config, options, factory);
	//scp.setDatabaseFlags(OFFalse, OFFalse, options.debug_);

	UnsethingQueryRetrieveSCP *localSCP = nil;
	
	localSCP = new UnsethingQueryRetrieveSCP(config, options, factory);
