...
NSString* const OsirixDataDirName = @"Unsething Data";
...
+(NSString*)baseDirPathForPath:(NSString*)path {
    NSString *existingPath = path.stringByStandardizingPath;
    while( existingPath.length > 1)
    {
        // Preserve an explicitly selected database directory even if its index is
        // temporarily absent or inaccessible. Never redirect it to a new archive.
        if( [@[@"unsething data", @"horos data", @"osirix data"] containsObject: existingPath.lastPathComponent.lowercaseString])
            return existingPath;
