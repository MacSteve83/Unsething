...
#ifdef WITH_OPENSSL
//static int         opt_keyFileFormat = DCF_Filetype_PEM;
//static OFBool      opt_doAuthenticate = OFFalse;
static OFString opt_ciphersuites("DEFAULT");
...
//    if (cond != EC_Normal) {
//        std::cerr << "error: updateStringAttributeValue: cannot find: " << tag.getTagName()
//             << " " << key << ": "
//             << cond.text() << endl;
//        return OFFalse;
//    }
...
//        std::cerr << "error: updateStringAttributeValue: INTERNAL ERROR: " << tag.getTagName()
...
//    if (cond != EC_Normal) {
//        std::cerr << "error: updateStringAttributeValue: cannot put string in attribute: " << tag.getTagName()
//             << " " << key << ": "
//             << cond.text() << endl;
//        return OFFalse;
//    }
...
//
//    if (opt_verbose) {
//        std::cout << "Inventing Identifying Information (" <<
//            "pa" << patientCounter << ", st" << studyCounter <<
//            ", se" << seriesCounter << ", im" << imageCounter << "): " << endl;
//        std::cout << "  PatientName=" << patientName << endl;
//        std::cout << "  PatientID=" << patientID << endl;
//        std::cout << "  StudyInstanceUID=" << studyInstanceUID << endl;
//        std::cout << "  StudyID=" << studyID << endl;
//        std::cout << "  SeriesInstanceUID=" << seriesInstanceUID << endl;
//        std::cout << "  SeriesNumber=" << seriesNumber << endl;
//        std::cout << "  SOPInstanceUID=" << sopInstanceUID << endl;
//        std::cout << "  ImageNumber=" << imageNumber << endl;
//    }
//
...
//	if ([fileManager createDirectoryAtPath:tempFolder attributes:nil]) NSLog(@"created Folder: %@", tempFolder);
	...
	DCM_dcmnetLogger.setLogLevel([[NSUserDefaults standardUserDefaults] boolForKey:@"verbose_dcmtkStoreScu"] ? OFLogger::DEBUG_LOG_LEVEL : OFLogger::WARN_LOG_LEVEL);
	
	DCM_dcmdataLogger.setLogLevel(opt_debug ? OFLogger::DEBUG_LOG_LEVEL : OFLogger::WARN_LOG_LEVEL);
            ...
            @synchronized( opensslSync)
            {
                @try
                {
                    ...
                    for (NSString *suite in _cipherSuites)
                    {
                        ...
                        if (NULL == (currentOpenSSL = DcmTLSCiphersuiteHandler::getOpenSSLCipherSuiteName(DcmTLSCiphersuiteHandler::lookupCiphersuite(current))))
                        {
                            NSLog(@"ciphersuite '%s' is unknown.", current);
                            NSLog(@"Known ciphersuites are:");
                            unsigned long numSuites = DcmTLSCiphersuiteHandler::getNumberOfCipherSuites();
...
//				 else std::cerr << "warning: " << errormsg << ", ignoring file" << endl;
			}
			else
			{
			  if (opt_proposeOnlyRequiredPresentationContexts)
		...
		if( _secureConnection)
        {
            @synchronized( opensslSync)
            {
                ...
                if(certVerification==VerifyPeerCertificate || certVerification==RequirePeerCertificate)
                {
                    ...
                    {
                        if (EC_Normal != tLayer->addTrustedCertificateFile([[trustedCertificatesDir stringByAppendingPathComponent:cert] cStringUsingEncoding:NSUTF8StringEncoding], static_cast<DcmKeyFileFormat>(_keyFileFormat)))
                        {
                            ...
                            //					{
                            //						std::cerr << "warning unable to load certificates from directory '" << current << "', ignoring" << endl;
                            //					}
                ...
                {			
                    ...
                    if (EC_Normal != tLayer->setPrivateKeyFile([_privateKeyFile cStringUsingEncoding:NSUTF8StringEncoding], DCF_Filetype_PEM))
                    {
                    ...
                    if (EC_Normal != tLayer->setCertificateFile([_certificateFile cStringUsingEncoding:NSUTF8StringEncoding], DCF_Filetype_PEM, tLayer->getTLSProfile()))
                    {
                ...
                if (EC_Normal != tLayer->setCipherSuites(opt_ciphersuites.c_str()))
                {
...
/*
    if (tLayer && opt_writeSeedFile)
    {
      if (tLayer->canWriteRandomSeed())
      {
        ...
        {
          std::cerr << "Error while writing random seed file '" << opt_writeSeedFile << "', ignoring." << endl;
        }
      } else {
        std::cerr << "Warning: cannot write random seed, ignoring." << endl;
      }
    }
    delete tLayer;
*/
