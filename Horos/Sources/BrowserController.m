...
- (void) subSelectFilesAndFoldersToAdd: (NSArray*) filenames
{
    ...
    else
    {
        ...
        for( int i = 0; i < [filenames count]; i++)
        {
            ...
            if ([[aPath pathExtension] isEqualToString:@"horosplugin"] || [[aPath pathExtension] isEqualToString:@"unsethingplugin"] || [[aPath pathExtension] isEqualToString:@"osirixplugin"])
...
-(IBAction) createDatabaseFolder:(id) sender
{
    ...
    [oPanel beginWithCompletionHandler:^(NSInteger result) {
        ...
        if( [[location lastPathComponent] isEqualToString:@"DATABASE.noindex"] && [[[location stringByDeletingLastPathComponent] lastPathComponent] isEqualToString:OsirixDataDirName])
...
- (NSString*)getDatabaseIndexFileFor: (NSString*)path {  // __deprecated
    ...
    if( [[NSFileManager defaultManager] fileExistsAtPath: path isDirectory: &isDirectory])
    {
        ...
        {
            // Default SQL file
            NSString	*index = [[path stringByAppendingPathComponent:OsirixDataDirName] stringByAppendingPathComponent:@"Database.sql"];
    ...
    if( albumTable.selectedRow > 0)
    {
        ...
        if( [albumArray count] > albumTable.selectedRow)
        {
            ...
            else
            {
                ...
                description = [description stringByAppendingFormat:NSLocalizedString(@"Album selected: %@", nil), [(DicomAlbum *)album displayName]];
            }
        }
    }
    ...
    @try
    {
        if ([aTableView isEqual:albumTable])
        {
            ...
            if (rowIndex >= 0 && rowIndex < albumArray.count) {
                id album = [albumArray objectAtIndex:rowIndex];
                [cell setTitle:[album isKindOfClass:DicomAlbum.class] ? [album displayName] : [album valueForKey:@"name"]];
            }
...
- (void)addHelpMenu
{
    ...
    NSMenuItem *communitySupportItem = [helpMenu addItemWithTitle: NSLocalizedString(@"Community support", nil) action: nil keyEquivalent: @""];
    [communitySupportItem setEnabled: NO];
    [helpMenu addItem: [NSMenuItem separatorItem]];
    NSMenuItem *reportBugItem = [helpMenu addItemWithTitle: NSLocalizedString(@"Report a bug", nil) action: nil keyEquivalent: @""];
    [reportBugItem setEnabled: NO];
    //[helpMenu addItem: [NSMenuItem separatorItem]];
...
- (void) setupToolbar
{
    ...
    toolbar = [[OsiriXToolbar alloc] initWithIdentifier: DatabaseToolbarIdentifier];
    ...
    // Keep filter controls inside their toolbar containers when AppKit moves
    // those containers into and out of the customization palette.
    for( NSView *filterView in @[timeIntervalView, modalityFilterView])
    {
        BOOL alreadyConstrained = NO;
        for( NSLayoutConstraint *constraint in [filterView constraints])
            if( [[constraint identifier] isEqualToString: @"UnsethingToolbarFilterCenterY"])
                alreadyConstrained = YES;
        if( alreadyConstrained)
            continue;

        for( NSView *control in [filterView subviews])
        {
            if( [control isKindOfClass: [NSPopUpButton class]] == NO)
                continue;
            [control setTranslatesAutoresizingMaskIntoConstraints: NO];
            NSLayoutConstraint *centerY = [[control centerYAnchor] constraintEqualToAnchor: [filterView centerYAnchor]];
            [centerY setIdentifier: @"UnsethingToolbarFilterCenterY"];
            [NSLayoutConstraint activateConstraints: @[
                centerY,
                [[control leadingAnchor] constraintEqualToAnchor: [filterView leadingAnchor] constant: 1],
                [[control trailingAnchor] constraintEqualToAnchor: [filterView trailingAnchor] constant: -1],
                [[control heightAnchor] constraintEqualToConstant: 22]
            ]];
        }
    }
...
- (NSToolbarItem *) toolbar: (NSToolbar *)toolbar itemForItemIdentifier: (NSString *) itemIdent willBeInsertedIntoToolbar:(BOOL) willBeInserted
{
    ...
    UnsethingApplyToolbarItemSizing(toolbarItem);
