 ...
 *
 *  Purpose: class UnsethingQueryRetrieveOptions
 *
    ...
    DIC_IS seriesNumber, DIC_CS modality, DIC_IS imageNumber)
{
    ...
    {
        UnsethingQueryRetrieveOptions::errmsg("Help!, cannot open image file: %s", imgFile);
        return;
    }
...
static void TI_buildStudyQuery(DcmDataset **query)
{
    ...
    *query = new DcmDataset;
    if (*query == NULL) {
        UnsethingQueryRetrieveOptions::errmsg("Help, out of memory");
        return;
    }
...
TI_addSeriesEntry(TI_StudyEntry *study, DcmDataset *reply)
{
    ...
    if (study->seriesCount >= TI_MAXSERIES) {
        UnsethingQueryRetrieveOptions::errmsg("TI_addSeriesEntry: too many series");
        return OFFalse;
    }
    ...
    if (!ok) {
        UnsethingQueryRetrieveOptions::errmsg("TI_addSeriesEntry: missing data in DB reply");
        return OFFalse;
    }
...
TI_addImageEntry(TI_SeriesEntry *series, DcmDataset *reply)
{
    ...
    if (series->imageCount >= TI_MAXIMAGES) {
        UnsethingQueryRetrieveOptions::errmsg("TI_addImageEntry: too many images");
        return OFFalse;
    }
    ...
    if (!ok) {
        UnsethingQueryRetrieveOptions::errmsg("TI_addImageEntry: missing data in DB reply");
        return OFFalse;
    }
...
TI_addStudyEntry(TI_DBEntry *db, DcmDataset *reply)
{
    ...
    if (db->studyCount >= TI_MAXSTUDIES) {
        UnsethingQueryRetrieveOptions::errmsg("TI_addStudyEntry: too many studies");
        return OFFalse;
    }
    ...
    if (!ok) {
        UnsethingQueryRetrieveOptions::errmsg("TI_addStudyEntry: missing data in DB reply");
        return OFFalse;
    }
...
/* ========================================== UnsethingQueryRetrieveTelnetInitiator ======================================== */

UnsethingQueryRetrieveTelnetInitiator::UnsethingQueryRetrieveTelnetInitiator(
      UnsethingQueryRetrieveConfig &cfg)
: dbEntries(NULL)
, dbCount(0)
, peerHostName(NULL)
, peerNamesCount(0)
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_detachAssociation(OFBool abortFlag)
{
    ...
    ASC_getPresentationAddresses(assoc->params, NULL, 0, presentationAddress, sizeof(presentationAddress));
    ...
    if (abortFlag) {
        ...
        if (cond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("Association Abort Failed:");
        ...
        if (cond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("Association Release Failed:");
            DimseCondition::dump(cond);
        }
    }
...
OFCondition UnsethingQueryRetrieveTelnetInitiator::addPresentationContexts(T_ASC_Parameters *params)
{
    OFCondition cond = EC_Normal;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_attachAssociation()
{
    OFCondition cond = EC_Normal;
    int port;
    const char *peer;
    ...
    if (cond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("Help, cannot create association parameters:");
        DimseCondition::dump(cond);
        return OFFalse;
    }
    ...
    if (!config.peerForAETitle(currentPeerTitle, &peer, &port)) {
        UnsethingQueryRetrieveOptions::errmsg("Help, AE title (%s) no longer in config",
            currentPeerTitle);
    ...
    if (cond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("Help, cannot add presentation contexts:");
        DimseCondition::dump(cond);
        ASC_destroyAssociationParameters(&params);
    ...
    if (debug) {
        ...
        ASC_dumpParameters(params, std::cout);
    }
    ...
    if (cond.bad()) {
        if (cond == DUL_ASSOCIATIONREJECTED) {
            ...
            UnsethingQueryRetrieveOptions::errmsg("Association Rejected:");
            ...
            UnsethingQueryRetrieveOptions::errmsg("Association Request Failed: Peer (%s, %s)",
    ...
    if (ASC_countAcceptedPresentationContexts(params) == 0) {
        UnsethingQueryRetrieveOptions::errmsg("All Presentation Contexts Refused: Peer (%s, %s)",
                presentationAddress, currentPeerTitle);
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_changeAssociation()
{
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_sendEcho()
{
    OFCondition cond = EC_Normal;
    DIC_US msgId;
    DIC_US status;
    ...
    if (cond.good()) {
        ...
        UnsethingQueryRetrieveOptions::errmsg("Failed:");
        DimseCondition::dump(cond);
        ASC_abortAssociation(assoc);
        ASC_dropAssociation(assoc);
        ASC_destroyAssociation(&assoc);
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_storeImage(char *sopClass, char *sopInstance, char * imgFile)
{
    OFCondition cond = EC_Normal;
    DIC_US msgId;
    DcmDataset *stDetail = NULL;
    ...
    if (presId == 0) {
        UnsethingQueryRetrieveOptions::errmsg("No presentation context for: (%s) %s",
    ...
    if (cond.good()) {
        ...
        UnsethingQueryRetrieveOptions::errmsg("[MsgID %d] Failed:", msgId);
        DimseCondition::dump(cond);
        ASC_abortAssociation(assoc);
        ASC_dropAssociation(assoc);
        ASC_destroyAssociation(&assoc);
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_remoteFindQuery(TI_DBEntry *db, DcmDataset *query,
    ...
    TI_GenericCallbackStruct *callbackData)
{
    ...
    presId = ASC_findAcceptedPresentationContextID(assoc,
        UID_FINDStudyRootQueryRetrieveInformationModel);
    if (presId == 0) {
        UnsethingQueryRetrieveOptions::errmsg("No Presentation Context for Find Operation");
        return OFFalse;
    }
    ...
    if (cond.good()) {
        ...
        UnsethingQueryRetrieveOptions::errmsg("Find Failed:");
        DimseCondition::dump(cond);
    }
    if (stDetail != NULL) {
        printf("  Status Detail:\n");
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_shortHelp(int /*arg*/ , const char * /*cmdbuf*/ )
{
    printf("h)elp, t)itle, da)tabase, st)udy, ser)ies i)mage, di)splay, sen)d, e)cho, q)uit\n");
    return OFTrue;
}

OFBool UnsethingQueryRetrieveTelnetInitiator::TI_help(int arg, const char * /*cmdbuf*/ )
{
    if (verbose) {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_title(int arg, const char * /*cmdbuf*/ )
{
    int i;
    TI_DBEntry *db;
    const char *peer;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_attachDB(TI_DBEntry *db)
{
    ...
    if (!db->isRemoteDB && db->dbHandle == NULL) {
        ...
        db->dbHandle = new UnsethingQueryRetrieveIndexDatabaseHandle(
            config.getStorageArea(db->title), 
            config.getMaxStudies(db->title),
            config.getMaxBytesPerStudy(db->title), dbcond);
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_attachDB: cannot create DB Handle");
            return OFFalse;
        }
    } else {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_database(int arg, const char * /*cmdbuf*/ )
{
    int i;
    TI_DBEntry *db = NULL;
    ...
    if (arg < 0) {
        ...
        if (arg >= dbCount) {
            ...
            {
                     UnsethingQueryRetrieveOptions::errmsg("ERROR: unable to open database, bailing out.\n");
                     exit(10);
            }
        }
    }
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_echo(int arg, const char * /*cmdbuf*/ )
{
    OFBool ok = OFTrue;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_quit(int arg, const char * /*cmdbuf*/ )
{
    if (verbose) {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_actualizeStudies()
{
    TI_DBEntry *db;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_study(int arg, const char * /*cmdbuf*/ )
{
    TI_DBEntry *db;
    TI_StudyEntry *se;
    int i;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_actualizeSeries()
{
    TI_DBEntry *db;
    TI_StudyEntry *study;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_series(int arg, const char * /*cmdbuf*/ )
{
    TI_DBEntry *db;
    TI_StudyEntry *study;
    TI_SeriesEntry *series;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_actualizeImages()
{
    TI_DBEntry *db;
    TI_StudyEntry *study;
    TI_SeriesEntry *series;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_image(int arg, const char * /*cmdbuf*/ )
{
    TI_DBEntry *db;
    TI_StudyEntry *study;
    TI_SeriesEntry *series;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_sendStudy(int arg, const char * /*cmdbuf*/ )
{
    OFBool ok = OFTrue;
    ...
    TI_StudyEntry *study;
    OFCondition dbcond = EC_Normal;
    UnsethingQueryRetrieveDatabaseStatus dbStatus(STATUS_Pending);
    DIC_UI sopClass;
    DIC_UI sopInstance;
    ...
    */
    ...
    if (dbcond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("TI_sendStudy: cannot query database");
        return OFFalse;
    }
    ...
    while (ok && dbStatus.status() == STATUS_Pending) {
        ...
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_sendStudy: database error");
            return OFFalse;
        }
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_sendSeries(int arg, const char * /*cmdbuf*/ )
{
    OFBool ok = OFTrue;
    DcmDataset *query = NULL;
    TI_DBEntry *db;
    ...
    delete query; query = NULL;
    if (dbcond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("TI_sendSeries: cannot query database");
        return OFFalse;
    }
    ...
    while (ok && dbStatus.status() == STATUS_Pending) {
        dbcond = db->dbHandle->nextMoveResponse(sopClass, sopInstance,
            imgFile, &nRemaining, &dbStatus);
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_sendSeries: database error");
            return OFFalse;
        }
        if (dbStatus.status() == STATUS_Pending) {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_sendImage(int arg, const char * /*cmdbuf*/ )
{
    OFBool ok = OFTrue;
    DcmDataset *query = NULL;
    TI_DBEntry *db;
    ...
    delete query; query = NULL;
    if (dbcond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("TI_sendImage: cannot query database");
        return OFFalse;
    }
    ...
    while (ok && dbStatus.status() == STATUS_Pending) {
      ...
      imgFile, &nRemaining, &dbStatus);
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_sendImage: database error");
            return OFFalse;
        }
        if (dbStatus.status() == STATUS_Pending) {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_send(int /*arg*/, const char *cmdbuf)
{
    OFBool ok = OFTrue;
    char cmdarg[128];
    int iarg;
...
void UnsethingQueryRetrieveTelnetInitiator::TI_userInput()
{
    ...
    while (1) {
        ...
        if (fgets(cmdBuf, 1024, stdin) == NULL) {
            UnsethingQueryRetrieveOptions::errmsg("unexpected end of input\n");
            return; /* give up */
        }
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_dbReadable(const char *dbTitle)
{
...
time_t UnsethingQueryRetrieveTelnetInitiator::TI_dbModifyTime(const char *dbTitle)
{
    char path[MAXPATHLEN+1];
    struct stat s;
    ...
    if (stat(path, &s) < 0) {
        UnsethingQueryRetrieveOptions::errmsg("cannot stat: %s", path);
        return 0;
    }
    return s.st_mtime;
}
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_buildRemoteStudies(TI_DBEntry *db)
{
    TI_GenericCallbackStruct cbs;
    DcmDataset *query = NULL;
    OFBool ok = OFTrue;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_buildStudies(TI_DBEntry *db)
{
    OFCondition dbcond = EC_Normal;
    UnsethingQueryRetrieveDatabaseStatus dbStatus(STATUS_Pending);
    DcmDataset *query = NULL;
    ...
    if (dbcond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("TI_buildStudies: cannot query database");
        delete query;
        return OFFalse;
    }
    ...
    while (dbStatus.status() == STATUS_Pending) {
        ...
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_buildStudies: database error");
            return OFFalse;
        }
        if (dbStatus.status() == STATUS_Pending) {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_buildRemoteSeries(TI_DBEntry *db, TI_StudyEntry *study)
{
    TI_GenericCallbackStruct cbs;
    DcmDataset *query = NULL;
    OFBool ok = OFTrue;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_buildSeries(TI_DBEntry *db, TI_StudyEntry *study)
{
    OFCondition dbcond = EC_Normal;
    UnsethingQueryRetrieveDatabaseStatus dbStatus(STATUS_Pending);
    DcmDataset *query = NULL;
    ...
    dbcond = db->dbHandle->startFindRequest(
        UID_FINDStudyRootQueryRetrieveInformationModel, query, &dbStatus);
    if (dbcond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("TI_buildSeries: cannot query database");
        delete query; query = NULL;
        return OFFalse;
    }
    ...
    while (dbStatus.status() == STATUS_Pending) {
        dbcond = db->dbHandle->nextFindResponse(&reply, &dbStatus);
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_buildSeries: database error");
            return OFFalse;
        }
        if (dbStatus.status() == STATUS_Pending) {
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_buildRemoteImages(TI_DBEntry *db, TI_StudyEntry *study, TI_SeriesEntry *series)
{
    TI_GenericCallbackStruct cbs;
    DcmDataset *query = NULL;
    OFBool ok = OFTrue;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::TI_buildImages(TI_DBEntry *db, TI_StudyEntry *study, TI_SeriesEntry *series)
{
    OFCondition dbcond = EC_Normal;
    UnsethingQueryRetrieveDatabaseStatus dbStatus(STATUS_Pending);
    DcmDataset *query = NULL;
    ...
    delete query; query = NULL;
    if (dbcond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("TI_buildImages: cannot query database");
        return OFFalse;
    }
    ...
    while (dbStatus.status() == STATUS_Pending) {
        dbcond = db->dbHandle->nextFindResponse(&reply, &dbStatus);
        if (dbcond.bad()) {
            UnsethingQueryRetrieveOptions::errmsg("TI_buildImages: database error");
            return OFFalse;
        }
        if (dbStatus.status() == STATUS_Pending) {
            if (verbose) {
                printf("REPLY OBJECT:\n");
                reply->print(std::cout);
            }
...
OFBool UnsethingQueryRetrieveTelnetInitiator::addPeerName(const char *peerName, const char *configFileName)
{
  int k;
  OFBool found = OFFalse;
  const char **aeTitles;
  ...
  {
    UnsethingQueryRetrieveOptions::errmsg("no AE titles defined (in: %s) for peer: %s", configFileName, peerName);
    return( OFFalse );
  }
...
void UnsethingQueryRetrieveTelnetInitiator::printConfig()
{
  int i,j;
...
OFBool UnsethingQueryRetrieveTelnetInitiator::findDBPeerTitles(
  ...
  const char *peer)
{
  ...
  if( peerTitleCount <= 0 )
  {
    UnsethingQueryRetrieveOptions::errmsg("no AE titles defined (in: %s) for peer: %s", configFileName, peer);
    return( OFFalse );
  }
...
void UnsethingQueryRetrieveTelnetInitiator::createConfigEntries(
  const char *configFileName,
  ...
  const char **remoteDBTitles)
{
  ...
  for( i=0 ; i<ctnTitleCount ; i++ )
  {
    if( !TI_dbReadable( ctnTitles[i] ) )
      UnsethingQueryRetrieveOptions::errmsg("Warning: db does not exist: %s", ctnTitles[i]);
    else
    {
  ...
  for( i=0 ; i<remoteDBTitlesCount ; i++ )
  {
    ...
    if( config.peerForAETitle( remoteDBTitles[i], &peerName, &portNumber ) )
    {
      ...
      for( j=0 ; j<peerNamesCount ; j++ )
      {
        ...
        if( peerTitleCount <= 0 )
          UnsethingQueryRetrieveOptions::errmsg("no AE titles defined (in: %s) for peer: %s", configFileName, peerNames[j]);
