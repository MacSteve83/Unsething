 ...
 *
 *  Purpose: class UnsethingQueryRetrieveMoveContext
 *
...
OFBool UnsethingQueryRetrieveMoveContext::isVerbose() const 
{ 
  return options_.verbose_ ? OFTrue : OFFalse; 
}

void UnsethingQueryRetrieveMoveContext::callbackHandler(
	/* in */ 
	OFBool cancelled, 
...
{
//	printf("UnsethingQueryRetrieveMoveContext::callbackHandler\n");
    OFCondition cond = EC_Normal;
    OFCondition dbcond = EC_Normal;
    UnsethingQueryRetrieveDatabaseStatus dbStatus(priorStatus);
	...
	if (options_.verbose_) {
        ...
        if (dbcond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("moveSCP: Database: startMoveRequest Failed (%s):",
...
void UnsethingQueryRetrieveMoveContext::addFailedUIDInstance(const char *sopInstance)
{
    int len;

    if (failedUIDs == NULL) {
	if ((failedUIDs = (char*)malloc(DIC_UI_LEN+1)) == NULL) {
	    UnsethingQueryRetrieveOptions::errmsg("malloc failure: addFailedUIDInstance");
	    return;
	}
	...
	if ((failedUIDs = (char*)realloc(failedUIDs, 
	    ...
	    UnsethingQueryRetrieveOptions::errmsg("realloc failure: addFailedUIDInstance");
	    return;
	}
...
OFCondition UnsethingQueryRetrieveMoveContext::performMoveSubOp(DIC_UI sopClass, DIC_UI sopInstance, char *fname)
{
    OFCondition cond = EC_Normal;
    T_DIMSE_C_StoreRQ req;
	...
	UnsethingQueryRetrieveOptions::errmsg("Move SCP: storeSCU: [file: %s]: %s", 
	...
	nFailed++;
	addFailedUIDInstance(sopInstance);
	UnsethingQueryRetrieveOptions::errmsg("Move SCP: storeSCU: [file: %s] No presentation context for: (%s) %s", 
	...
	if (rsp.DimseStatus == STATUS_Success) {
	    ...
	    nWarning++;
	    UnsethingQueryRetrieveOptions::errmsg("Move SCP: Store Waring: Response Status: %s", 
		DU_cstoreStatusString(rsp.DimseStatus));
	} else {
	    nFailed++;
	    addFailedUIDInstance(sopInstance);
	    /* print a status message */
	    UnsethingQueryRetrieveOptions::errmsg("Move SCP: Store Failed: Response Status: %s", 
		DU_cstoreStatusString(rsp.DimseStatus));
	}
    } else {
	nFailed++;
	addFailedUIDInstance(sopInstance);
	UnsethingQueryRetrieveOptions::errmsg("Move SCP: storeSCU: Store Request Failed:");
...
OFCondition UnsethingQueryRetrieveMoveContext::buildSubAssociation(T_DIMSE_C_MoveRQ *request)
{
    OFCondition cond = EC_Normal;
    ...
    if (cond.good())
	{
		...
		if (cond.bad())
		{
			UnsethingQueryRetrieveOptions::errmsg("moveSCP: Cannot create Association-params for sub-ops:");
			DimseCondition::dump(cond);
		}
    }
	...
	if (cond.bad()) {
	    if (cond == DUL_ASSOCIATIONREJECTED) {
			...
			UnsethingQueryRetrieveOptions::errmsg("moveSCP: Sub-Association Rejected");
			ASC_printRejectParameters(stderr, &rej);
			fprintf(stderr, "\n");
	    } else {
			UnsethingQueryRetrieveOptions::errmsg("moveSCP: Sub-Association Request Failed:");
			DimseCondition::dump(cond);
...
OFCondition UnsethingQueryRetrieveMoveContext::closeSubAssociation()
{
    OFCondition cond = EC_Normal;
	...
	if (cond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("moveSCP: Sub-Association Release Failed:");
	    DimseCondition::dump(cond);
	}
	cond = ASC_dropAssociation(subAssoc);
	if (cond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("moveSCP: Sub-Association Drop Failed:");
	    DimseCondition::dump(cond);
	}
	cond = ASC_destroyAssociation(&subAssoc);
	if (cond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("moveSCP: Sub-Association Destroy Failed:");
	    DimseCondition::dump(cond);
	}
...
void UnsethingQueryRetrieveMoveContext::moveNextImage(UnsethingQueryRetrieveDatabaseStatus * dbStatus)
{
    OFCondition cond = EC_Normal;
    OFCondition dbcond = EC_Normal;
	...
	*/
    ...
    if (dbcond.bad())
	{
		UnsethingQueryRetrieveOptions::errmsg("moveSCP: Database: nextMoveResponse Failed (%s):",DU_cmoveStatusString(dbStatus->status()));
    }
    ...
    if (dbStatus->status() == STATUS_Pending)
	{
		...
		if (cond != EC_Normal)
		{
			UnsethingQueryRetrieveOptions::errmsg("moveSCP: Move Sub-Op Failed:");
			DimseCondition::dump(cond);
...
void UnsethingQueryRetrieveMoveContext::failAllSubOperations(UnsethingQueryRetrieveDatabaseStatus * dbStatus)
{
    OFCondition dbcond = EC_Normal;
    DIC_UI subImgSOPClass;	/* sub-operation image SOP Class */
    DIC_UI subImgSOPInstance;	/* sub-operation image SOP Instance */
    ...
    while (dbStatus->status() == STATUS_Pending) {
	    ...
	    &nRemaining, dbStatus);
        if (dbcond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("moveSCP: Database: nextMoveResponse Failed (%s):",
	        DU_cmoveStatusString(dbStatus->status()));
        }
...
void UnsethingQueryRetrieveMoveContext::buildFailedInstanceList(DcmDataset ** rspIds)
{
    OFBool ok;
	...
	*rspIds = new DcmDataset();
	ok = DU_putStringDOElement(*rspIds, DCM_FailedSOPInstanceUIDList,
	    failedUIDs);
	if (!ok) {
	    UnsethingQueryRetrieveOptions::errmsg("moveSCP: failed to build DCM_FailedSOPInstanceUIDList");
	}
	free(failedUIDs);
	failedUIDs = NULL;
    }
...
OFBool UnsethingQueryRetrieveMoveContext::mapMoveDestination(
			...
			if (tsIndex)
			{
				switch ([tsIndex intValue])
				{
					...
					case SendJPEGLSLossy50: preferredTS = EXS_JPEGLSLossy;
                        break;
					case SendJPEGLossless: preferredTS = EXS_JPEGProcess14SV1;
						break;
					case SendJPEGLossy9:
					case SendJPEGLossy8:
					case SendJPEGLossy7: preferredTS = EXS_JPEGProcess2_4;
						break;
...
//OFCondition UnsethingQueryRetrieveMoveContext::addAllStoragePresentationContexts(T_ASC_Parameters *params, E_TransferSyntax preferredSyntax){
...
OFCondition UnsethingQueryRetrieveMoveContext::addAllStoragePresentationContexts(T_ASC_Parameters *params)
{
    OFCondition cond = EC_Normal;
