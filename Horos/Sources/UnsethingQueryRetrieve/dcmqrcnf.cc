 ...
 *
 *  Purpose: class UnsethingQueryRetrieveConfig
 *
...
int UnsethingQueryRetrieveConfig::aeTitlesForPeer(const char *hostName, const char *** aeTitleList) const
{
    int n = 0;
    int i, j, k;
...
int UnsethingQueryRetrieveConfig::ctnTitles(const char *** ctnTitleList) const
{
    int i;
    int n = 0;
...
int UnsethingQueryRetrieveConfig::aeTitlesForSymbolicName(const char *symbolicName, const char *** aeTitleList) const
{
    int i = 0;
    int j = 0;
    int n = 0;
...
const char *UnsethingQueryRetrieveConfig::vendorForPeerAETitle(const char *peerAETitle) const
{
    int i = 0;
    int j = 0;
...
int UnsethingQueryRetrieveConfig::countCtnTitles() const
{
    return CNF_Config.noOfAEEntries;
}


void UnsethingQueryRetrieveConfig::initConfigStruct() 
{
...
void UnsethingQueryRetrieveConfig::panic(const char *fmt, ...)
{
   va_list  ap;
...
int UnsethingQueryRetrieveConfig::readConfigLines(FILE *cnffp)
{
...
int UnsethingQueryRetrieveConfig::readHostTable(FILE *cnffp, int *lineno)
{
   ...
   UnsethingQueryRetrieveConfigHostEntry *helpentry;
   ...
   while (!feof(cnffp)) {
      ...
      if ((helpentry = (UnsethingQueryRetrieveConfigHostEntry *)malloc(CNF_HETable.noOfHostEntries * sizeof(UnsethingQueryRetrieveConfigHostEntry))) == NULL)
         panic("Memory allocation 1 (%d)", CNF_HETable.noOfHostEntries);
      if (CNF_HETable.noOfHostEntries - 1) {
         memcpy((char*)helpentry, (char*)CNF_HETable.HostEntries, (CNF_HETable.noOfHostEntries - 1) *sizeof(UnsethingQueryRetrieveConfigHostEntry));
...
int UnsethingQueryRetrieveConfig::readVendorTable(FILE *cnffp, int *lineno)
{
   int  error = 0,        /* error flag */
        end = 0,          /* end flag */
        noOfPeers;        /* number of peers for entry */
   ...
   while (!feof(cnffp)) {
      ...
      if ((helpentry = (UnsethingQueryRetrieveConfigHostEntry *)malloc(CNF_VendorTable.noOfHostEntries * sizeof(UnsethingQueryRetrieveConfigHostEntry))) == NULL)
         panic("Memory allocation 2 (%d)", CNF_VendorTable.noOfHostEntries);
      if (CNF_VendorTable.noOfHostEntries - 1) {
         memcpy((char*)helpentry, (char*)CNF_VendorTable.HostEntries, (CNF_VendorTable.noOfHostEntries - 1) *sizeof(UnsethingQueryRetrieveConfigHostEntry));
...
int UnsethingQueryRetrieveConfig::readAETable(FILE *cnffp, int *lineno)
{
   int  error = 0,          /* error flag */
        end = 0,            /* end flag */
   ...
   char rcline[512],        /* line in configuration file */
        ...
        value[256],         /* parameter value */
        *lineptr;           /* pointer to line */
   UnsethingQueryRetrieveConfigAEEntry *helpentry;
   ...
   while (!feof(cnffp)) {
      ...
      lineptr = rcline;
      noOfAEEntries++;
      if ((helpentry = (UnsethingQueryRetrieveConfigAEEntry *)malloc(noOfAEEntries * sizeof(UnsethingQueryRetrieveConfigAEEntry))) == NULL)
         panic("Memory allocation 3 (%d)", noOfAEEntries);
      if (noOfAEEntries - 1) {
         memcpy((char*)helpentry, (char*)CNF_Config.AEEntries, (noOfAEEntries - 1) *sizeof(UnsethingQueryRetrieveConfigAEEntry));
...
UnsethingQueryRetrieveConfigQuota *UnsethingQueryRetrieveConfig::parseQuota(char **valuehandle)
{
   int  studies;
   char *helpvalue,
        helpval[20];
   UnsethingQueryRetrieveConfigQuota *helpquota;

   if ((helpquota = (UnsethingQueryRetrieveConfigQuota *)malloc(sizeof(UnsethingQueryRetrieveConfigQuota))) == NULL)
...
UnsethingQueryRetrieveConfigPeer *UnsethingQueryRetrieveConfig::parsePeers(char **valuehandle, int *peers)
{
   char *helpvalue;
   ...
   if (!strcmp("ANY", helpvalue)) {     /* keywork ANY used */
      free(helpvalue);
      *peers = -1;
      return((UnsethingQueryRetrieveConfigPeer *) 0);
   }
...
UnsethingQueryRetrieveConfigPeer *UnsethingQueryRetrieveConfig::readPeerList(char **valuehandle, int *peers)
{
   int  i,
   found,
   noOfPeers = 0;
   char *helpvalue;
   UnsethingQueryRetrieveConfigPeer *helppeer,
   *peerlist = NULL;
   ...
   while((helpvalue = parsevalues(valuehandle)) != NULL) {
      ...
      if (strchr(helpvalue, ',') == NULL) {   /* symbolic name */
        ...
        if ((helppeer = (UnsethingQueryRetrieveConfigPeer *)malloc(noOfPeers * sizeof(UnsethingQueryRetrieveConfigPeer))) == NULL)
            panic("Memory allocation 5 (%d)", noOfPeers);
        if (noOfPeers - CNF_HETable.HostEntries[i].noOfPeers) {
            memcpy((char*)helppeer, (char*)peerlist, (noOfPeers - CNF_HETable.HostEntries[i].noOfPeers) * sizeof(UnsethingQueryRetrieveConfigPeer));
            free(peerlist);
         }
         peerlist = helppeer;
         memcpy((char*)(peerlist + (noOfPeers - CNF_HETable.HostEntries[i].noOfPeers)), (char*)CNF_HETable.HostEntries[i].Peers, CNF_HETable.HostEntries[i].noOfPeers * sizeof(UnsethingQueryRetrieveConfigPeer));
      }
         ...
         if (noOfPeers - 1) {
            memcpy((char*)helppeer, (char*)peerlist, (noOfPeers - 1) *sizeof(UnsethingQueryRetrieveConfigPeer));
            free(peerlist);
         }
         peerlist = helppeer;
...
char *UnsethingQueryRetrieveConfig::skipmnemonic (char *rcline)
{
   char *help = rcline;
...
int UnsethingQueryRetrieveConfig::isgap (char gap)
{
...
int UnsethingQueryRetrieveConfig::isquote (char quote)
{
...
char *UnsethingQueryRetrieveConfig::parsevalues (char **valuehandle)
{
   int i,
       inquotes = 0,
       count = 0;
...
long UnsethingQueryRetrieveConfig::quota (const char *value)
{
   int  number;
   long factor;
...
int UnsethingQueryRetrieveConfig::init(const char *ConfigurationFile)
{
   int  error = 0;        /* error flag */
...
void UnsethingQueryRetrieveConfig::printConfig()
{
   int i,j;
...
const char *UnsethingQueryRetrieveConfig::getApplicationTitle() const
{
   return(applicationTitle_.c_str());
}


const char *UnsethingQueryRetrieveConfig::getApplicationContext() const
{
   return(applicationContext_.c_str());
}


const char *UnsethingQueryRetrieveConfig::getImplementationClass() const
{
   return(implementationClass_.c_str());
}


const char *UnsethingQueryRetrieveConfig::getImplementationVersion() const
{
   return(implementationVersion_.c_str());
}


const char *UnsethingQueryRetrieveConfig::getNetworkType() const
{
   return(networkType_.c_str());
}


int UnsethingQueryRetrieveConfig::getNetworkTCPPort() const
{
   return(networkTCPPort_);
}


OFCmdUnsignedInt UnsethingQueryRetrieveConfig::getMaxPDUSize() const
{
   return(maxPDUSize_);
}


int UnsethingQueryRetrieveConfig::getMaxAssociations() const
{
   return(maxAssociations_);
}


const char *UnsethingQueryRetrieveConfig::getStorageArea(const char *AETitle) const
{
   int  i;
...
const char *UnsethingQueryRetrieveConfig::getAccess(const char *AETitle) const
{
   int  i;
...
int UnsethingQueryRetrieveConfig::getMaxStudies(const char *AETitle) const
{
   int  i;
...
long UnsethingQueryRetrieveConfig::getMaxBytesPerStudy(const char *AETitle) const
{
   int  i;
...
int UnsethingQueryRetrieveConfig::peerInAETitle(const char *calledAETitle, const char *callingAETitle, const char *HostName) const
{
   int  i,
    j;
...
int UnsethingQueryRetrieveConfig::peerForAETitle(const char *AETitle, const char **HostName, int *PortNumber) const
{
   int  i,
    j;
...
int UnsethingQueryRetrieveConfig::checkForSameVendor(const char *AETitle1, const char *AETitle2) const
{
   int  i,
    j,
    k,
...
int UnsethingQueryRetrieveConfig::HostNamesForVendor(const char *Vendor, const char ***HostNameArray) const
{
   int  i, j,
    found = 0;
...
OFBool UnsethingQueryRetrieveConfig::writableStorageArea(const char *aeTitle) const
{
...
const char *UnsethingQueryRetrieveConfig::getUserName() const
{
   return UserName_.c_str();
}

const char *UnsethingQueryRetrieveConfig::getGroupName() const
{
