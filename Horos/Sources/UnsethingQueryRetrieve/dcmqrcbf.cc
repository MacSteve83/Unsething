 ...
 *
 *  Purpose: class UnsethingQueryRetrieveFindContext
 *
...
void UnsethingQueryRetrieveFindContext::callbackHandler(
	/* in */ 
	...
	DcmDataset **stDetail)
{
	...
	if (options_.verbose_) {
        ...
        if (dbcond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("findSCP: Database: startFindRequest Failed (%s):",
	...
	if (dbcond.bad()) {
	     UnsethingQueryRetrieveOptions::errmsg("findSCP: Database: nextFindResponse Failed (%s):",
		 DU_cfindStatusString(dbStatus.status()));
	}
    }
    ...
    if (*responseIdentifiers != NULL)
    {
	  ...
	  {
		UnsethingQueryRetrieveOptions::errmsg("DO Error: adding Retrieve AE Title");
      }
    }
