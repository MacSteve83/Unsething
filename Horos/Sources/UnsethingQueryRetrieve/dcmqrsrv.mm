 ...
 *
 *  Purpose: class UnsethingQueryRetrieveSCP
 *
...
#include "dcfilefo.h"
#include "dcmqrdba.h"
#include "dcmqrcbf.h"    /* for class UnsethingQueryRetrieveFindContext */
#include "dcmqrcbm.h"    /* for class UnsethingQueryRetrieveMoveContext */
#include "dcmqrcbg.h"    /* for class UnsethingQueryRetrieveGetContext */
#include "dcmqrcbs.h"    /* for class UnsethingQueryRetrieveStoreContext */
#include "dcmetinf.h"
#include "dul.h"
#import "dcmqrdbq.h"
...
+ (void) waitUnlockFileWithPID: (NSDictionary*) dict
{
    ...
    if (cond.bad())
    {
        //UnsethingQueryRetrieveOptions::errmsg("Cannot Drop Association:");
    ...
    if (cond.bad())
    {
        //UnsethingQueryRetrieveOptions::errmsg("Cannot Destroy Association:");
        DimseCondition::dump(cond);
    }
...
+ (void) handleAssociation: (NSDictionary*) d
{
    ...
    @try
    {
        ...
        UnsethingQueryRetrieveSCP *scp = (UnsethingQueryRetrieveSCP*) [[d valueForKey: @"UnsethingQueryRetrieveSCP"] pointerValue];
...
static void findCallback(
        ...
        DcmDataset **stDetail)
{
  UnsethingQueryRetrieveFindContext *context = OFstatic_cast(UnsethingQueryRetrieveFindContext *, callbackData);
...
static void getCallback(
        ...
        DcmDataset **responseIdentifiers)
{
  UnsethingQueryRetrieveGetContext *context = OFstatic_cast(UnsethingQueryRetrieveGetContext *, callbackData);
...
static void moveCallback(
        ...
        DcmDataset **responseIdentifiers)
{
  UnsethingQueryRetrieveMoveContext *context = OFstatic_cast(UnsethingQueryRetrieveMoveContext *, callbackData);
  context->callbackHandler(cancelled, request, requestIdentifiers, responseCount, response, stDetail, responseIdentifiers);
...
static void storeCallback(
    ...
    DcmDataset **stDetail)
{
  UnsethingQueryRetrieveStoreContext *context = OFstatic_cast(UnsethingQueryRetrieveStoreContext *, callbackData);
  context->callbackHandler(progress, req, imageFileName, context->sourceAETitle, context->destinationAETitle, imageDataSet, rsp, stDetail);
}
...
UnsethingQueryRetrieveSCP::UnsethingQueryRetrieveSCP(
  const UnsethingQueryRetrieveConfig& config,
  const UnsethingQueryRetrieveOptions& options,
  const UnsethingQueryRetrieveDatabaseHandleFactory& factory)
: config_(&config)
...
void UnsethingQueryRetrieveSCP::lockFile(void)
{
...
void UnsethingQueryRetrieveSCP::unlockFile(void)
{
    if( options_.singleProcess_)
        return;
...
void UnsethingQueryRetrieveSCP::writeStateProcess( const char *str, T_ASC_Association *assoc)
{
    if( options_.singleProcess_)
    {
...
void UnsethingQueryRetrieveSCP::writeErrorMessage( const char *str)
{
    if( options_.singleProcess_)
    {
        if( str)
...
NSString* UnsethingQueryRetrieveSCP::getErrorMessage()	// see emptyDeleteQueue: for reading this error message
{
	NSString *str = [NSString stringWithContentsOfFile: @"/tmp/error_message" usedEncoding:NULL error:NULL];
...
//void UnsethingQueryRetrieveSCP::waitUnlockFileWithPID(int pid)
//{
//	BOOL fileExist = YES;
...
OFCondition UnsethingQueryRetrieveSCP::dispatch(T_ASC_Association *assoc, OFBool correctUIDPadding)
{
    OFCondition cond = EC_Normal;
    T_DIMSE_Message msg;
    ...
    while (cond.good())
    {
        ...
        UnsethingQueryRetrieveDatabaseHandle *dbHandle = nil;
...
//        @synchronized( globalSync)
        ...
        if (cond.bad())
        {
          UnsethingQueryRetrieveOptions::errmsg("dispatch: cannot create DB Handle");
          return cond;
        }
...
//            @synchronized( globalSync)
            ...
            if (cond.good())
            {
                ...
                {
                    UnsethingQueryRetrieveOptions::errmsg("CGET NOT ACTIVATED - Cannot handle command: 0x%x\n", (unsigned)msg.CommandField);
                    ms = DIMSE_NOTHING;
                }
                
                if( ms == DIMSE_C_FIND_RQ && activateCFINDSCP_ == false)
                {
                    UnsethingQueryRetrieveOptions::errmsg("CFIND NOT ACTIVATED - Cannot handle command: 0x%x\n", (unsigned)msg.CommandField);
                    ms = DIMSE_NOTHING;
                }
                ...
                default:
                    ...
                    UnsethingQueryRetrieveOptions::errmsg("Cannot handle command: 0x%x\n", (unsigned)msg.CommandField);
...
OFCondition UnsethingQueryRetrieveSCP::handleAssociation(T_ASC_Association * assoc, OFBool correctUIDPadding)
{
    OFCondition           cond = EC_Normal;
 ...
 /* now do the real work */
    ...
    {
        UnsethingQueryRetrieveOptions::errmsg("DIMSE Failure (aborting association):\n");
        DimseCondition::dump(cond);
...
OFCondition UnsethingQueryRetrieveSCP::echoSCP(T_ASC_Association * assoc, T_DIMSE_C_EchoRQ * req,
        T_ASC_PresentationContextID presId)
{
    OFCondition cond = EC_Normal;
    ...
    if (cond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("echoSCP: Echo Response Failed:");
        DimseCondition::dump(cond);
    }
    return cond;
}
...
OFCondition UnsethingQueryRetrieveSCP::findSCP(T_ASC_Association * assoc, T_DIMSE_C_FindRQ * request,
        T_ASC_PresentationContextID presID,
        UnsethingQueryRetrieveDatabaseHandle& dbHandle)

{
    OFCondition cond = EC_Normal;
    UnsethingQueryRetrieveFindContext context(dbHandle, options_, STATUS_Pending);
    ...
    if (cond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("Find SCP Failed:");
        DimseCondition::dump(cond);
    }
    return cond;
}
...
OFCondition UnsethingQueryRetrieveSCP::getSCP(T_ASC_Association * assoc, T_DIMSE_C_GetRQ * request,
        T_ASC_PresentationContextID presID, UnsethingQueryRetrieveDatabaseHandle& dbHandle)
{
    OFCondition cond = EC_Normal;
    UnsethingQueryRetrieveGetContext context(dbHandle, options_, STATUS_Pending, assoc, request->MessageID, request->Priority, presID);
    ...
    if (cond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("Get SCP Failed:");
        DimseCondition::dump(cond);
    }
    return cond;
}
...
OFCondition UnsethingQueryRetrieveSCP::moveSCP(T_ASC_Association * assoc, T_DIMSE_C_MoveRQ * request,
        T_ASC_PresentationContextID presID, UnsethingQueryRetrieveDatabaseHandle& dbHandle)
{
    OFCondition cond = EC_Normal;
	//printf("move context\n");
    UnsethingQueryRetrieveMoveContext context(dbHandle, options_, NULL, STATUS_Pending, assoc, request->MessageID, request->Priority);
    ...
    if (cond.bad()) {
        UnsethingQueryRetrieveOptions::errmsg("Move SCP Failed:");
        DimseCondition::dump(cond);
    }
    return cond;
}
...
OFCondition UnsethingQueryRetrieveSCP::storeSCP(T_ASC_Association * assoc, T_DIMSE_C_StoreRQ * request,
             T_ASC_PresentationContextID presId,
             UnsethingQueryRetrieveDatabaseHandle& dbHandle,
             OFBool correctUIDPadding)
{
    ...
    UnsethingQueryRetrieveStoreContext context(dbHandle, options_, STATUS_Success, &dcmff, correctUIDPadding);
    // Preserve AE metadata for both C-GET suboperations and incoming C-STORE.
    if (assoc->params->DULparams.respondingAPTitle[0]) {
        context.sourceAETitle = assoc->params->DULparams.respondingAPTitle;
        context.destinationAETitle = assoc->params->DULparams.callingAPTitle;
    } else {
        context.sourceAETitle = assoc->params->DULparams.callingAPTitle;
        context.destinationAETitle = assoc->params->DULparams.calledAPTitle;
    }
    ...
    if (!dcmIsaStorageSOPClassUID(request->AffectedSOPClassUID)) {
		...
		if (dbcond.bad())
		{
            UnsethingQueryRetrieveOptions::errmsg("storeSCP: Database: makeNewStoreFileName Failed");
            /* must still receive data */
            strcpy(imageFileName, NULL_DEVICE_NAME);
    ...
    if (lockfd < 0)
    {
        UnsethingQueryRetrieveOptions::errmsg("storeSCP: file locking failed, cannot create file");
	...
	static_cast<UnsethingQueryRetrieveOsiriXDatabaseHandle *>(&dbHandle) -> updateLogEntry(dset);

    if (cond.bad())
	{
        UnsethingQueryRetrieveOptions::errmsg("Store SCP Failed:");
        DimseCondition::dump(cond);
...
void UnsethingQueryRetrieveSCP::refuseAnyStorageContexts(T_ASC_Association * assoc)
{
    int i;
...
OFCondition UnsethingQueryRetrieveSCP::refuseAssociation(T_ASC_Association ** assoc, CTN_RefuseReason reason)
{
    OFCondition cond = EC_Normal;
...
OFCondition UnsethingQueryRetrieveSCP::negotiateAssociation(T_ASC_Association * assoc)
{
    OFCondition cond = EC_Normal;
    int i;
    ...
    if (cond.bad()) {
    UnsethingQueryRetrieveOptions::errmsg("Cannot accept presentation contexts:");
    DimseCondition::dump(cond);
    }
	...
	*/
    ...
    for (i=0; i<(int)DIM_OF(queryRetrievePairs); i++) {
        ...
        if (options_.requireFindForMove_) {
            ...
            UnsethingQueryRetrieveOptions::errmsg("WARNING: Move PresCtx but no Find (accepting for now)");
        }
        }
        }
    }
...
OFCondition UnsethingQueryRetrieveSCP::waitForAssociation(T_ASC_Network * theNet)
{
    OFCondition cond = EC_Normal;
#ifdef HAVE_FORK
    int                 pid;
...
//      }
    ...
    if (ASC_associationWaiting(theNet, timeout))
    {
        ...
        if (cond.bad())
        {
          if (options_.verbose_)
          {
            UnsethingQueryRetrieveOptions::errmsg("Failed to receive association:");
            DimseCondition::dump(cond);
          }
          go_cleanup = OFTrue;
        }
    ...
    if (! go_cleanup)
    {
        ...
        {
          printf("Parameters:\n");
          ASC_dumpParameters(assoc->params, std::cout);
        }
    ...
    if (! go_cleanup)
    {
        ...
        if (cond.bad() || strcmp(buf, DICOM_STDAPPLICATIONCONTEXT) != 0)
        {
            ...
            if (options_.verbose_)
            {
                UnsethingQueryRetrieveOptions::errmsg("Bad AppContextName: %s", buf);
            }
    ...
    if (! go_cleanup)
    {
        ...
        if (options_.rejectWhenNoImplementationClassUID_ && strlen(assoc->params->theirImplementationClassUID) == 0)
        {
            ...
            if (options_.verbose_)
            {
                UnsethingQueryRetrieveOptions::errmsg("No implementation Class UID provided");
            }
            cond = refuseAssociation(&assoc, CTN_NoReason);
            go_cleanup = OFTrue;
        }
	...
	if(! go_cleanup)
	{
		if( [[DicomDatabase activeLocalDatabase] isFileSystemFreeSizeLimitReached])
		{
            ...
            if (options_.verbose_)
            {
                UnsethingQueryRetrieveOptions::errmsg("No enough memory on the hard disk");
            }
            cond = refuseAssociation(&assoc, CTN_NoReason);
            go_cleanup = OFTrue;
		}
...
//            else
            ...
            if( cond != ASC_SHUTDOWNAPPLICATION)
            {
                if (singleProcess) // But multi-threaded
                {
                    ...
                    @try
                    {
                        NSThread *t = [[[NSThread alloc] initWithTarget: [ContextCleaner class] selector:@selector(handleAssociation:) object: [NSDictionary dictionaryWithObjectsAndKeys: [NSValue valueWithPointer: assoc], @"assoc", [NSValue valueWithPointer: this], @"UnsethingQueryRetrieveSCP", nil]] autorelease];
                ...
                else
                {
                    ...
                    @try
                    {
                        ...
                        if (pid < 0)
                        {
                            ...
                            UnsethingQueryRetrieveOptions::errmsg("Cannot create association sub-process: %s", strerror(errno));
...
void UnsethingQueryRetrieveSCP::cleanChildren(OFBool verbose)
{
//  processtable_.cleanChildren(verbose);
}


void UnsethingQueryRetrieveSCP::setDatabaseFlags(
...
void UnsethingQueryRetrieveSCP::setSecureConnection(OFBool secureConnection)
{
 ...
 * Passing calling aetitle to UnsethingQueryRetrieveDatabaseHandleFactory::createDBHandle
