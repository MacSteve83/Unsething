 ...
 *
 *  Purpose: classes UnsethingQueryRetrieveIndexDatabaseHandle, UnsethingQueryRetrieveIndexDatabaseHandleFactory
 *
...
static const DB_FindAttr TbFindAttr [] = {
        ...
        DB_FindAttr( DCM_RETIRED_OtherPatientIDs,                       PATIENT_LEVEL,  OPTIONAL_KEY,   STRING_CLASS    ),
        DB_FindAttr( DCM_OtherPatientNames,                     PATIENT_LEVEL,  OPTIONAL_KEY,   STRING_CLASS    ),
        DB_FindAttr( DCM_RETIRED_EthnicGroup,                           PATIENT_LEVEL,  OPTIONAL_KEY,   STRING_CLASS    ),
                ...
                )
{
    ...
    if (plist == NULL) {
        std::cerr << "DB_UIDAddFound: out of memory" << std::endl;
        return;
    }
...
static void DB_IdxInitRecord (IdxRecord *idx, int linksOnly)
{
    if (! linksOnly)
    {
        ...
        idx -> PatientID[0] = '\0' ;
        idx -> param[RECORDIDX_PatientsBirthTime]. XTag = DCM_PatientBirthTime  ;
        ...
        idx -> PatientsAge[0] = '\0' ;
        idx -> param[RECORDIDX_PatientsSize]. XTag = DCM_PatientSize  ;
        idx -> param[RECORDIDX_PatientsSize]. ValueLength = DS_MAX_LENGTH ;
        idx -> PatientsSize[0] = '\0' ;
        idx -> param[RECORDIDX_PatientsWeight]. XTag = DCM_PatientWeight  ;
...
static long DB_lseek(int fildes, long offset, int whence)
{
    ...
    */
    if (offset < 0) {
        std::cerr << "*** DB ALERT: attempt to seek before begining of file" << std::endl;
    }

    /* get the current position */
    curpos = lseek(fildes, 0, SEEK_CUR);
    if (curpos < 0) {
        std::cerr << "DB_lseek: cannot get current position: " << strerror(errno) << std::endl;
        return curpos;
    }
    /* get the end of file position */
    endpos = lseek(fildes, 0, SEEK_END);
    if (endpos < 0) {
        std::cerr << "DB_lseek: cannot get end of file position: " << strerror(errno) << std::endl;
        return endpos;
    }
    ...
    if (curpos < 0) {
        std::cerr << "DB_lseek: cannot reset current position: " << strerror(errno) << std::endl;
        return curpos;
    }
    ...
    */
    ...
    if (pos > maxFileSize) {
        std::cerr << "*** DB ALERT: attempt to seek beyond " << maxFileSize << " bytes" << std::endl;
    }
    ...
    if ((endpos > 0) && (pos > endpos)) {
        std::cerr << "*** DB ALERT: attempt to seek beyond end of file" << std::endl
            << "              offset=" << offset << " filesize=" << endpos << std::endl;
    }
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_IdxRead (int idx, IdxRecord *idxRec)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_StudyDescChange(StudyDescRecord *pStudyDesc)
{
    OFCondition cond = EC_Normal;
    DB_lseek (handle -> pidx, 0L, SEEK_SET) ;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_IdxInitLoop(int *idx)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_IdxGetNext(int *idx, IdxRecord *idxRec)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_GetStudyDesc (StudyDescRecord *pStudyDesc)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_IdxRemove(int idx)
{
    IdxRecord   rec ;
    OFCondition cond = EC_Normal;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_lock(OFBool exclusive)
{
    int lockmode;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::DB_unlock()
{
...
static void DB_DuplicateElement (DB_SmallDcmElmt *src, DB_SmallDcmElmt *dst)
{
    ...
    if (dst->PValueField != NULL) {
        ...
        std::cerr << "DB_DuplicateElement: out of memory" << std::endl;
    }
    }
}
...
UnsethingQueryRetrieveDatabaseHandle::~UnsethingQueryRetrieveDatabaseHandle()
{
}
...
int UnsethingQueryRetrieveIndexDatabaseHandle::matchDate (DB_SmallDcmElmt *mod, DB_SmallDcmElmt *elt)
{
...
int UnsethingQueryRetrieveIndexDatabaseHandle::matchTime (DB_SmallDcmElmt *mod, DB_SmallDcmElmt *elt)
{
...
int UnsethingQueryRetrieveIndexDatabaseHandle::matchUID (DB_SmallDcmElmt *mod, DB_SmallDcmElmt *elt)
{
    int match ;
    char *uid  ;
    char *modl  ;
...
int UnsethingQueryRetrieveIndexDatabaseHandle::matchStrings (DB_SmallDcmElmt *mod, DB_SmallDcmElmt *elt)
{
    int match ;
    char *string  ;
    char *modl  ;
...
int UnsethingQueryRetrieveIndexDatabaseHandle::matchOther (DB_SmallDcmElmt *mod, DB_SmallDcmElmt *elt)
{
...
int UnsethingQueryRetrieveIndexDatabaseHandle::dbmatch (DB_SmallDcmElmt *mod, DB_SmallDcmElmt *elt)
{
...
void UnsethingQueryRetrieveIndexDatabaseHandle::makeResponseList (
                DB_Private_Handle       *phandle,
                IdxRecord               *idxRec
                )
{
    ...
    **/
    ...
    for (pRequestList = phandle->findRequestList ; pRequestList ; pRequestList = pRequestList->next) {
        ...
        **/
        ...
        if (plist == NULL) {
            std::cerr << "makeResponseList: out of memory" << std::endl;
            return;
        }
        plist->next = NULL ;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::testFindRequestList (
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::hierarchicalCompare (
                DB_Private_Handle       *phandle,
                IdxRecord               *idxRec,
                DB_LEVEL                level,
                DB_LEVEL                infLevel,
                int                     *match)
{
    ...
    ***/
    ...
    if (level < phandle->queryLevel) {
        ...
        if (plist == NULL) {
            *match = OFFalse ;
            std::cerr << "hierarchicalCompare : No UID Key found at level " << (int) level << std::endl;
            return DcmQRIndexDatabaseError ;
        }
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::startFindRequest(
                const char      *SOPClassUID,
                DcmDataset      *findRequestIdentifiers,
                UnsethingQueryRetrieveDatabaseStatus  *status)
{
    ...
    ***/
    ...
    if (!qrLevelFound) {
        ...
        std::cerr << "DB_startFindRequest(): missing Query/Retrieve Level" << std::endl;
        handle->idxCounter = -1 ;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::nextFindResponse (
        ...
        **/
        ...
        for ( plist = handle->findResponseList ; plist != NULL ; plist = plist->next ) {
            ...
            if (plist->elem.PValueField != NULL &&
                ...
                if (ec != EC_Normal) {
                    std::cerr << "dbfind: DB_nextFindResponse: cannot put()" << std::endl;
                    status->setStatus(STATUS_FIND_Failed_UnableToProcess);
                    return DcmQRIndexDatabaseError;
                }
            }
            ...
            if (ec != EC_Normal) {
                std::cerr << "dbfind: DB_nextFindResponse: cannot insert()" << std::endl;
                status->setStatus(STATUS_FIND_Failed_UnableToProcess);
                return DcmQRIndexDatabaseError;
            }
        }
...
#ifdef DEBUG
        if (debugLevel > 0) {
            std::cout << "DB: findResponseIdentifiers:" << std::endl;
            (*findResponseIdentifiers)->print(std::cout);
        }
#endif
    }
    else {
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::cancelFindRequest (UnsethingQueryRetrieveDatabaseStatus *status)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::testMoveRequestList (
                DB_ElementList  *findRequestList,
                DB_LEVEL        queryLevel,
                DB_LEVEL        infLevel,
                DB_LEVEL        lowestLevel
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::startMoveRequest(
        const char      *SOPClassUID,
        ...
        UnsethingQueryRetrieveDatabaseStatus  *status)
{
    ...
    ***/
    ...
    if (!qrLevelFound) {
        ...
        std::cerr << "DB_startMoveRequest(): missing Query/Retrieve Level" << std::endl;
        handle->idxCounter = -1 ;
        DB_FreeElementList (handle->findRequestList) ;
        handle->findRequestList = NULL ;
        return (DcmQRIndexDatabaseError) ;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::nextMoveResponse(
                char            *SOPClassUID,
                char            *SOPInstanceUID,
                char            *imageFileName,
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::cancelMoveRequest (UnsethingQueryRetrieveDatabaseStatus *status)
{
...
void UnsethingQueryRetrieveIndexDatabaseHandle::enableQuotaSystem(OFBool enable)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::deleteImageFile(char* imgFile)
{
    if (!quotaSystemEnabled) {
    std::cerr << "DB QUOTA: disabled: retaining: " << imgFile << std::endl;
    return EC_Normal;
    } else {
    std::cerr << "DB QUOTA: enabled: deleting: " << imgFile << std::endl;
    }
...
#endif
    if (lockfd < 0) {
    std::cerr << "DB ERROR: cannot open image file for deleting: " << imgFile << std::endl;
    return DcmQRIndexDatabaseError;
    }
    if (dcmtk_flock(lockfd, LOCK_EX) < 0) { /* exclusive lock (blocking) */
    std::cerr << "DB ERROR: cannot lock image file  for deleting: " << imgFile << std::endl;
    ...
    if (unlink(imgFile) < 0) {
        /* delete file */
    std::cerr << "DB ERROR: cannot delete image file: " << imgFile << std::endl
        << "DcmQRIndexDatabaseError: " << strerror(errno) << std::endl;
   }

#ifdef LOCK_IMAGE_FILES
    if (dcmtk_flock(lockfd, LOCK_UN) < 0) { /* unlock */
    std::cerr << "DB ERROR: cannot unlock image file  for deleting: " << imgFile << std::endl;
        dcmtk_plockerr("DB ERROR");
     }
...
int UnsethingQueryRetrieveIndexDatabaseHandle::deleteOldestStudy(StudyDescRecord *pStudyDesc)
{
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::deleteOldestImages(StudyDescRecord *pStudyDesc, int StudyNum, char *StudyUID, long RequiredSize)
{
    ...
    if (StudyArray == NULL) {
    std::cerr << "deleteOldestImages: out of memory" << std::endl;
    return DcmQRIndexDatabaseError;
    }
...
int UnsethingQueryRetrieveIndexDatabaseHandle::matchStudyUIDInStudyDesc (StudyDescRecord *pStudyDesc, char *StudyUID, int maxStudiesAllowed)
{
    int s = 0 ;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::checkupinStudyDesc(StudyDescRecord *pStudyDesc, char *StudyUID, long imageSize)
{
    int         s ;
    long        RequiredSize ;
 ...
 * hewett - Nov. 1, 93
 */
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::removeDuplicateImage(
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::storeRequest (
    const char  *SOPClassUID,
    ...
    OFBool      isNew)
{
    ...
    ***/
    ...
    {
      std::cerr << "DB: Cannot open file: " << imageFileName << ": "
    ...
    ***/
    ...
    if (pStudyDesc == NULL) {
      std::cerr << "DB_storeRequest: out of memory" << std::endl;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::pruneInvalidRecords()
{
    int idx = 0;
    IdxRecord idxRec ;
    StudyDescRecord *pStudyDesc;
    ...
    pStudyDesc = (StudyDescRecord *)malloc (SIZEOF_STUDYDESC) ;
    if (pStudyDesc == NULL) {
      std::cerr << "DB_pruneInvalidRecords: out of memory" << std::endl;
      DB_unlock();
      return (DcmQRIndexDatabaseError) ;
    }
...
void UnsethingQueryRetrieveIndexDatabaseHandle::printIndexFile (char *storeArea)
{
    int i ;
    int j ;
    IdxRecord           idxRec ;
    ...
    OFCondition result;
    UnsethingQueryRetrieveIndexDatabaseHandle handle(storeArea, -1, -1, result);
    if (result.bad()) return;

    pStudyDesc = (StudyDescRecord *)malloc (SIZEOF_STUDYDESC) ;
    if (pStudyDesc == NULL) {
        std::cerr << "printIndexFile: out of memory" << std::endl;
        return;
    }
    ...
    for (i=0; i<handle.handle->maxStudiesAllowed; i++) {
        if (pStudyDesc[i].NumberofRegistratedImages != 0 ) {
            std::cout << "******************************************************" << std::endl
                << "STUDY DESCRIPTOR: " << i << std::endl
                << "  Study UID: " << pStudyDesc[i].StudyInstanceUID << std::endl
                << "  StudySize: " << pStudyDesc[i].StudySize << std::endl
                << "  LastRecDate: " << pStudyDesc[i].LastRecordedDate << std::endl
                << "  NumOfImages: " << pStudyDesc[i].NumberofRegistratedImages << std::endl;
        }
    }
    ...
    while (1) {
        ...
        std::cout << "*******************************************************" << std::endl;
        std::cout << "RECORD NUMBER: " << j << std::endl << "  Status: ";
        if (idxRec.hstat == DVIF_objectIsNotNew)
            std::cout << "is NOT new" << std::endl;
        else
            std::cout << "is new" << std::endl;
        std::cout << "  Filename: " << idxRec.filename << std::endl
             << "  ImageSize: " << idxRec.ImageSize << std::endl
             << "  RecordedDate: " << idxRec.RecordedDate << std::endl;
        for (i = 0 ; i < NBPARAMETERS ; i++) {  /* new definition */
            DB_SmallDcmElmt *se = idxRec.param + i;
            const char* value = "";
            if (se->PValueField != NULL) value = se->PValueField;
            ...
            std::cout << "  InstanceDescription: \"" << idxRec.InstanceDescription << "\"" << std::endl;
    }
    std::cout << "*******************************************************" << std::endl
         << "RECORDS IN THIS INDEXFILE: " << j << std::endl;
...
const char *UnsethingQueryRetrieveIndexDatabaseHandle::getStorageArea() const
{
  return handle->storageArea;
}

const char *UnsethingQueryRetrieveIndexDatabaseHandle::getIndexFilename() const
{
  return handle->indexFilename;
}

void UnsethingQueryRetrieveIndexDatabaseHandle::setDebugLevel(int dLevel)
{
    debugLevel = dLevel;
}

int UnsethingQueryRetrieveIndexDatabaseHandle::getDebugLevel() const
{
    return debugLevel;
}

void UnsethingQueryRetrieveIndexDatabaseHandle::dbdebug(int level, const char* format, ...) const
{
    va_list ap;
...
void UnsethingQueryRetrieveIndexDatabaseHandle::setIdentifierChecking(OFBool checkFind, OFBool checkMove)
{
...
UnsethingQueryRetrieveIndexDatabaseHandle::UnsethingQueryRetrieveIndexDatabaseHandle(
...
, debugLevel(0)
{
    ...
    if (maxStudiesPerStorageArea > DB_UpperMaxStudies) {
        std::cerr << "WARING: maxStudiesPerStorageArea too large" << std::endl
             << "        setting to " << DB_UpperMaxStudies << std::endl;
    ...
    if (handle) {
        ...
        if (f == NULL) {
            std::cerr << handle->indexFilename << ": " << strerror(errno) << std::endl;
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::makeNewStoreFileName(
                const char      *SOPClassUID,
                const char      * /* SOPInstanceUID */ ,
...
OFCondition UnsethingQueryRetrieveIndexDatabaseHandle::instanceReviewed(int idx)
{
...
UnsethingQueryRetrieveIndexDatabaseHandleFactory::UnsethingQueryRetrieveIndexDatabaseHandleFactory(const UnsethingQueryRetrieveConfig *config)
: UnsethingQueryRetrieveDatabaseHandleFactory()
, config_(config)
{
}

UnsethingQueryRetrieveIndexDatabaseHandleFactory::~UnsethingQueryRetrieveIndexDatabaseHandleFactory()
{
}

UnsethingQueryRetrieveDatabaseHandle *UnsethingQueryRetrieveIndexDatabaseHandleFactory::createDBHandle(
    const char * /* callingAETitle */,
    const char *calledAETitle,
    OFCondition& result) const
{
  return new UnsethingQueryRetrieveIndexDatabaseHandle(
