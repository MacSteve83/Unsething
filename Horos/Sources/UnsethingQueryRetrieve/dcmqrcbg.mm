...
OFCondition UnsethingQueryRetrieveGetContext::performGetSubOp(DIC_UI sopClass, DIC_UI sopInstance, char *fname)
{
    ...
    if (presId == 0)
	{
		nFailed++;
		addFailedUIDInstance(sopInstance);
		UnsethingQueryRetrieveOptions::errmsg("Get SCP: storeSCU: [file: %s] No presentation context for: (%s) %s", fname, dcmSOPClassUIDToModality(sopClass), sopClass);
...
void UnsethingQueryRetrieveGetContext::getNextImage(UnsethingQueryRetrieveDatabaseStatus * dbStatus)
{
    ...
    if (dbcond.bad())
	{
		UnsethingQueryRetrieveOptions::errmsg("getSCP: Database: nextMoveResponse Failed (%s):", DU_cmoveStatusString(dbStatus->status()));
    }
