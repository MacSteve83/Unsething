 ...
 *
 *  Purpose: class UnsethingQueryRetrieveGetContext
 *
...
OFBool UnsethingQueryRetrieveGetContext::isVerbose() const 
{ 
  return options_.verbose_ ? OFTrue : OFFalse;
}

void UnsethingQueryRetrieveGetContext::callbackHandler(
	/* in */ 
	...
	DcmDataset **responseIdentifiers)
{
	...
	if (options_.verbose_) {
        ...
        if (dbcond.bad()) {
	    UnsethingQueryRetrieveOptions::errmsg("getSCP: Database: startMoveRequest Failed (%s):",
...
void UnsethingQueryRetrieveGetContext::addFailedUIDInstance(const char *sopInstance)
{
    int len;
...
OFCondition UnsethingQueryRetrieveGetContext::performGetSubOp(DIC_UI sopClass, DIC_UI sopInstance, char *fname)
{
	...
	UnsethingQueryRetrieveOptions::errmsg("Get SCP: storeSCU: [file: %s]: %s", 
	...
	nFailed++;
	addFailedUIDInstance(sopInstance);
	UnsethingQueryRetrieveOptions::errmsg("Get SCP: storeSCU: [file: %s] No presentation context for: (%s) %s", 
	...
	return DIMSE_NOVALIDPRESENTATIONCONTEXTID;
        ...
        if ((pc.acceptedRole != ASC_SC_ROLE_SCP) && (pc.acceptedRole != ASC_SC_ROLE_SCUSCP)) {
            ...
            nFailed++;
	    addFailedUIDInstance(sopInstance);
	    UnsethingQueryRetrieveOptions::errmsg("Get SCP: storeSCU: [file: %s] No presentation context with requestor SCP role for: (%s) %s", 
	        fname, dcmSOPClassUIDToModality(sopClass), sopClass);
	    return DIMSE_NOVALIDPRESENTATIONCONTEXTID;
        }
    }
    ...
    if (cond.good()) {
        if (cancelParameters.cancelEncountered) {
        	...
        	UnsethingQueryRetrieveOptions::errmsg("Get SCP: Unexpected C-Cancel-RQ encountered: pid=%d, mid=%d", 
	...
	if (rsp.DimseStatus == STATUS_Success) {
	    ...
	    nWarning++;
	    UnsethingQueryRetrieveOptions::errmsg("Get SCP: Store Warning: Response Status: %s", 
		DU_cstoreStatusString(rsp.DimseStatus));
        } else {
	    nFailed++;
	    addFailedUIDInstance(sopInstance);
	    /* print a status message */
	    UnsethingQueryRetrieveOptions::errmsg("Get SCP: Store Failed: Response Status: %s", 
		DU_cstoreStatusString(rsp.DimseStatus));
	}
    } else {
	nFailed++;
	addFailedUIDInstance(sopInstance);
	UnsethingQueryRetrieveOptions::errmsg("Get SCP: storeSCU: Store Request Failed:");
...
void UnsethingQueryRetrieveGetContext::getNextImage(UnsethingQueryRetrieveDatabaseStatus * dbStatus)
{
    OFCondition cond = EC_Normal;
    OFCondition dbcond = EC_Normal;
	...
	&nRemaining, dbStatus);
    if (dbcond.bad()) {
	UnsethingQueryRetrieveOptions::errmsg("getSCP: Database: nextMoveResponse Failed (%s):",
	    DU_cmoveStatusString(dbStatus->status()));
    }
	...
	cond = performGetSubOp(subImgSOPClass,
        ...
        if (cond != EC_Normal) {
	    UnsethingQueryRetrieveOptions::errmsg("getSCP: Get Sub-Op Failed:");
	    DimseCondition::dump(cond);
...
void UnsethingQueryRetrieveGetContext::buildFailedInstanceList(DcmDataset ** rspIds)
{
    OFBool ok;
	...
	*rspIds = new DcmDataset();
	ok = DU_putStringDOElement(*rspIds, DCM_FailedSOPInstanceUIDList,
	    failedUIDs);
	if (!ok) {
	    UnsethingQueryRetrieveOptions::errmsg("getSCP: failed to build DCM_FailedSOPInstanceUIDList");
	}
	free(failedUIDs);
	failedUIDs = NULL;
    }
