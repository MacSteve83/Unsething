 ...
 *
 *  Purpose: classes UnsethingQueryRetrieveProcessSlot, UnsethingQueryRetrieveProcessTable
 *
 *  Last Update:      $Author: lpysher $
 *  Update Date:      $Date: 2006/03/01 20:16:07 $
 *  Source File:      $Source: /cvsroot/osirix/osirix/Binaries/dcmtk-source/dcmqrdb/dcmqrptb.cc,v $
 ...
 */
class UnsethingQueryRetrieveProcessSlot
{
public:
  /** constructor.
   ...
   */
  UnsethingQueryRetrieveProcessSlot(
    const char *peerName,
  ...
  /// destructor
  virtual ~UnsethingQueryRetrieveProcessSlot() {}
...
UnsethingQueryRetrieveProcessSlot::UnsethingQueryRetrieveProcessSlot(
      const char *peerName,
      const char *callingAETitle,
      const char *calledAETitle,
      int processId,
...
OFBool UnsethingQueryRetrieveProcessSlot::isProcessWithWriteAccess(const char *calledAETitle) const
{
...
UnsethingQueryRetrieveProcessTable::~UnsethingQueryRetrieveProcessTable()
{
  OFListIterator(UnsethingQueryRetrieveProcessSlot *) first = table_.begin();
  OFListIterator(UnsethingQueryRetrieveProcessSlot *) last = table_.end();
  while (first != last)
  {
    delete *first;
...
void UnsethingQueryRetrieveProcessTable::addProcessToTable(int pid, T_ASC_Association * assoc)
{
    ...
    UnsethingQueryRetrieveProcessSlot *slot = new UnsethingQueryRetrieveProcessSlot(
...
void UnsethingQueryRetrieveProcessTable::removeProcessFromTable(int pid)
{
  OFListIterator(UnsethingQueryRetrieveProcessSlot *) first = table_.begin();
  OFListIterator(UnsethingQueryRetrieveProcessSlot *) last = table_.end();
  while (first != last)
...
OFBool UnsethingQueryRetrieveProcessTable::haveProcessWithWriteAccess(const char *calledAETitle) const
{
  OFListConstIterator(UnsethingQueryRetrieveProcessSlot *) first = table_.begin();
  OFListConstIterator(UnsethingQueryRetrieveProcessSlot *) last = table_.end();
  while (first != last)
  {
...
void UnsethingQueryRetrieveProcessTable::cleanChildren(OFBool verbose)
{
    ...
    while (child > 0)
    {
      ...
      if (child < 0)
      {
          ...
          {
            ofConsole.lockCerr() << "error: wait for child process failed: " << strerror(errno) << std::endl;
...
//          {
//            time_t t = time(NULL);
//            ofConsole.lockCerr() << "Cleaned up after child (" << child << ") " << ctime(&t) << std::endl;
//            ofConsole.unlockCerr();
//          }
