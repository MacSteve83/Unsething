...
static NSString *SRDocumentString(const DSRDocument *document,
    OFCondition (DSRDocument::*getter)(OFString &, const signed long) const)
{
    OFString value;
    if ((document->*getter)(value, 0).bad()) return @"";
...
+ (NSString*) getImageRefSOPInstanceUID:(NSString*) path;
{
	...
	{			
		...
		if (status.good())
		{
			...
			int instanceNumber = [SRDocumentString(document, &DSRDocument::getInstanceNumber) intValue];
...
+ (NSString*) getReportFilenameFromSR:(NSString*) path;
{
	...
	{			
		...
		if (status.good())
		{
			status = document->read(*fileformat.getDataset());
			// See DicomFile.m
//			int frameNumber = [SRDocumentString(document, &DSRDocument::getInstanceNumber) intValue];
			NSString *accessionNumber = SRDocumentString(document, &DSRDocument::getAccessionNumber);
			NSString *studyInstanceUID = SRDocumentString(document, &DSRDocument::getStudyInstanceUID);
			NSString *patientName = SRDocumentString(document, &DSRDocument::getPatientName);
			NSString *patientID = SRDocumentString(document, &DSRDocument::getPatientID);
			NSString *patientDOB =  SRDocumentString(document, &DSRDocument::getPatientBirthDate);
...
{
	if (!_seriesInstanceUID)
		_seriesInstanceUID =  [SRDocumentString(document, &DSRDocument::getSeriesInstanceUID) retain];
...
- (NSString *)sopInstanceUID{
	return SRDocumentString(document, &DSRDocument::getSOPInstanceUID);
}

- (NSString *)sopClassUID{
	return SRDocumentString(document, &DSRDocument::getSOPClassUID);
}

- (NSString *)seriesDescription{

	return SRDocumentString(document, &DSRDocument::getSeriesDescription);
}

- (NSString *)seriesNumber{
	return SRDocumentString(document, &DSRDocument::getSeriesNumber);
}
@end
