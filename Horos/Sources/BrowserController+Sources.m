...
@interface BrowserSourcesHelper : NSObject<NSNetServiceBrowserDelegate, NSNetServiceDelegate>/*<NSTableViewDelegate,NSTableViewDataSource>*/
{
    ...
    NSMetadataQuery *_localDatabaseQuery;
    NSMutableSet *_discoveredDatabasePaths;
    BOOL _localDatabaseScanRunning;
    BOOL _localDatabaseScanPending;
}

-(id)initWithBrowser:(BrowserController*)browser;
-(void)_analyzeVolumeAtPath:(NSString*)path;
-(void)refreshLocalDatabaseDiscovery;
-(void)scanLocalDatabaseLocations:(NSArray*)volumes;
-(void)mergeDiscoveredLocalDatabases:(NSArray*)paths;
-(void)localDatabaseQueryUpdated:(NSNotification*)notification;
...
-(void)selectCurrentDatabaseSource
{
    ...
    {
        NSDictionary* source = [NSDictionary dictionaryWithObjectsAndKeys: _database.baseDirPath, @"Path", [_database.baseDirPath.stringByDeletingLastPathComponent.lastPathComponent stringByAppendingString: NSLocalizedString( @" DB", @"DB = DataBase")], @"Description", nil];
...
-(void)setDatabaseThread:(NSArray*)io
{
    ...
    @try
    {
        ...
        if ([type isEqualToString:@"Local"])
        {
            ...
            NSString *basePath = [DicomDatabase baseDirPathForPath:path];
            if( [@[@"horos data", @"osirix data"] containsObject: basePath.lastPathComponent.lowercaseString])
            {
                NSString *indexPath = [basePath stringByAppendingPathComponent:@"Database.sql"];
                BOOL isDirectory = NO;
                if( ![[NSFileManager defaultManager] fileExistsAtPath:indexPath isDirectory:&isDirectory]
                    || isDirectory || ![[NSFileManager defaultManager] isReadableFileAtPath:indexPath])
                    [NSException raise:NSGenericException format:@"Impossibile aprire l'archivio: indice Database.sql assente o non leggibile in %@. Verificare il percorso e i permessi del disco.", basePath];
            }
            db = [DicomDatabase databaseAtPath:basePath name:name];
            if( !db || !db.managedObjectContext.persistentStoreCoordinator.persistentStores.count)
                [NSException raise:NSGenericException format:@"Impossibile aprire l'archivio %@. Verificare i permessi e la compatibilita del database; il database corrente rimane selezionato.", basePath];
        }
...
-(id)initWithBrowser:(BrowserController*)browser
{
    if ((self = [super init]))
    {
        ...
        _discoveredDatabasePaths = [[NSMutableSet alloc] init];
        _localDatabaseQuery = [[NSMetadataQuery alloc] init];
        [_localDatabaseQuery setPredicate: [NSPredicate predicateWithFormat: @"kMDItemFSName == %@", @"Database.sql"]];
        [_localDatabaseQuery setSearchScopes: @[NSMetadataQueryLocalComputerScope]];
        [[NSNotificationCenter defaultCenter] addObserver: self selector: @selector(localDatabaseQueryUpdated:) name: NSMetadataQueryDidFinishGatheringNotification object: _localDatabaseQuery];
        [[NSNotificationCenter defaultCenter] addObserver: self selector: @selector(localDatabaseQueryUpdated:) name: NSMetadataQueryDidUpdateNotification object: _localDatabaseQuery];
        [_localDatabaseQuery startQuery];
        [self refreshLocalDatabaseDiscovery];
...
-(void)dealloc
{
    ...
    [[NSNotificationCenter defaultCenter] removeObserver: self name: NSMetadataQueryDidFinishGatheringNotification object: _localDatabaseQuery];
    [[NSNotificationCenter defaultCenter] removeObserver: self name: NSMetadataQueryDidUpdateNotification object: _localDatabaseQuery];
    [_localDatabaseQuery stopQuery];
    [_localDatabaseQuery release];
    [_discoveredDatabasePaths release];
...
// Discovery reads directory entries and the SQLite header only; databases are opened by the existing
// source-selection path, never by the scanner.
static BOOL UnsethingIsExistingDatabaseDirectory(NSString *path)
{
    if( ![@[@"Unsething Data", @"Horos Data", @"OsiriX Data"] containsObject: path.lastPathComponent])
        return NO;
    NSFileManager *fm = [NSFileManager defaultManager];
    BOOL isDirectory = NO;
    NSString *sqlPath = [path stringByAppendingPathComponent: @"Database.sql"];
    if( ![fm fileExistsAtPath: sqlPath isDirectory: &isDirectory] || isDirectory)
        return NO;
    BOOL hasImageDirectory = NO;
    for( NSString *name in @[@"DATABASE.noindex", @"DATABASE"])
        if( [fm fileExistsAtPath: [path stringByAppendingPathComponent: name] isDirectory: &isDirectory] && isDirectory)
            hasImageDirectory = YES;
    if( !hasImageDirectory)
        return NO;
    FILE *file = fopen(sqlPath.fileSystemRepresentation, "rb");
    if( file == NULL)
        return NO;
    char header[16];
    size_t length = fread(header, 1, sizeof(header), file);
    fclose(file);
    return length == sizeof(header) && memcmp(header, "SQLite format 3", sizeof(header)) == 0;
}

-(void)refreshLocalDatabaseDiscovery
{
    if( _localDatabaseScanRunning)
    {
        _localDatabaseScanPending = YES;
        return;
    }
    _localDatabaseScanRunning = YES;
    NSArray *volumes = [[NSFileManager defaultManager] mountedVolumeURLsIncludingResourceValuesForKeys: @[NSURLVolumeIsLocalKey] options: NSVolumeEnumerationSkipHiddenVolumes];
    [self performSelectorInBackground: @selector(scanLocalDatabaseLocations:) withObject: volumes];
}
...
-(void)scanLocalDatabaseLocations:(NSArray*)volumes
{
    @autoreleasepool
    {
        ...
        NSFileManager *fm = [NSFileManager defaultManager];
        NSMutableArray *roots = [NSMutableArray arrayWithObjects: NSHomeDirectory(), [NSHomeDirectory() stringByAppendingPathComponent: @"Documents"], nil];
        for( NSURL *volume in volumes)
        {
            ...
            if( !local.boolValue)
                continue;
            [roots addObject: volume.path];
            NSString *users = [volume.path stringByAppendingPathComponent: @"Users"];
            for( NSString *user in [fm contentsOfDirectoryAtPath: users error: nil])
                if( ![user hasPrefix: @"."])
                    [roots addObject: [[users stringByAppendingPathComponent: user] stringByAppendingPathComponent: @"Documents"]];
        }
        // Spotlight covers indexed locations at any depth. This small fallback
        // also finds databases on unindexed USB disks and in common folders.
        ...
        {
            NSString *path = [[queue[i] objectAtIndex: 0] stringByResolvingSymlinksInPath];
            NSInteger depth = [[queue[i] objectAtIndex: 1] integerValue];
            if( [visited containsObject: path])
                continue;
            [visited addObject: path];
            if( UnsethingIsExistingDatabaseDirectory(path))
            {
            ...
            if( depth >= 3)
                continue;
            NSArray *children = [fm contentsOfDirectoryAtURL: [NSURL fileURLWithPath: path] includingPropertiesForKeys: @[NSURLIsDirectoryKey, NSURLIsSymbolicLinkKey, NSURLIsPackageKey] options: NSDirectoryEnumerationSkipsHiddenFiles error: nil];
            for( NSURL *child in children)
            {
                if( [@[@"System", @"Library", @"Applications", @"Volumes", @"node_modules"] containsObject: child.lastPathComponent])
                    continue;
                NSDictionary *values = [child resourceValuesForKeys: @[NSURLIsDirectoryKey, NSURLIsSymbolicLinkKey, NSURLIsPackageKey] error: nil];
                if( [values[NSURLIsDirectoryKey] boolValue] && ![values[NSURLIsSymbolicLinkKey] boolValue] && ![values[NSURLIsPackageKey] boolValue])
                    [queue addObject: @[child.path, @(depth + 1)]];
            }
        }
        [self performSelectorOnMainThread: @selector(localDatabaseScanFinished:) withObject: paths.allObjects waitUntilDone: NO];
    }
}

-(void)localDatabaseScanFinished:(NSArray*)paths
{
    _localDatabaseScanRunning = NO;
    [self mergeDiscoveredLocalDatabases: paths];
    if( _localDatabaseScanPending)
    {
        _localDatabaseScanPending = NO;
        [self refreshLocalDatabaseDiscovery];
    }
}
...
-(void)localDatabaseQueryUpdated:(NSNotification*)notification
{
    [_localDatabaseQuery disableUpdates];
    NSMutableArray *paths = [NSMutableArray array];
    for( NSMetadataItem *item in [_localDatabaseQuery results])
    {
        NSString *path = [[item valueForAttribute: NSMetadataItemPathKey] stringByDeletingLastPathComponent];
        if( path)
            [paths addObject: path];
    }
    [_localDatabaseQuery enableUpdates];
    [self mergeDiscoveredLocalDatabases: paths];
}

-(void)mergeDiscoveredLocalDatabases:(NSArray*)paths
{
    BOOL previousListeningState = dontListenToSourcesChanges;
    dontListenToSourcesChanges = YES;
    NSString *defaultPath = [DicomDatabase.defaultDatabase.baseDirPath stringByResolvingSymlinksInPath];
    for( NSString *candidate in paths)
    {
        NSString *path = [candidate stringByResolvingSymlinksInPath];
        if( [path isEqualToString: defaultPath] || !UnsethingIsExistingDatabaseDirectory(path))
            continue;
        if( [_browser rowForSourceIdentifier: [LocalDatabaseNodeIdentifier localDatabaseNodeIdentifierWithPath: path]] >= 0)
            continue;
        NSURL *url = [NSURL fileURLWithPath: path];
        NSString *volumeName = nil;
        [url getResourceValue: &volumeName forKey: NSURLVolumeNameKey error: nil];
        NSString *label = [NSString stringWithFormat: @"%@ DB - %@", path.stringByDeletingLastPathComponent.lastPathComponent, volumeName ?: @"Locale"];
        LocalDatabaseNodeIdentifier *node = [LocalDatabaseNodeIdentifier localDatabaseNodeIdentifierWithPath: path description: label dictionary: @{@"icon": @"Unsething.icns"}];
        node.detected = YES;
        [_discoveredDatabasePaths addObject: path];
        [_browser.sources addObject: node];
    }
    for( DataNodeIdentifier *node in [[_browser.sources.content copy] autorelease])
    {
        if( ![_discoveredDatabasePaths containsObject: node.location] || UnsethingIsExistingDatabaseDirectory(node.location))
            continue;
        if( [node isEqualToDataNodeIdentifier: [_browser sourceIdentifierForDatabase: _browser.database]])
            continue;
        [_discoveredDatabasePaths removeObject: node.location];
        node.detected = NO;
        if( !node.entered)
            [_browser.sources removeObject: node];
    }
    [_browser selectCurrentDatabaseSource];
    [_browser redrawSources];
    dontListenToSourcesChanges = previousListeningState;
}

-(void)_analyzeVolumeAtPath:(NSString*)path
{
    NSMutableArray *databases = [NSMutableArray array];
    for( NSString *name in @[@"Unsething Data", @"Horos Data", @"OsiriX Data"])
    {
        NSString *candidate = [path stringByAppendingPathComponent: name];
        if( UnsethingIsExistingDatabaseDirectory(candidate))
            [databases addObject: candidate];
    }
    if( databases.count)
    {
        [self mergeDiscoveredLocalDatabases: databases];
        return; // An existing archive is a database source, not a DICOM import disk.
    }
...
-(void)willDisplayCell:(PrettyCell*)cell
{
    ...
    cell.image = [NSImage imageNamed:@"Unsething.icns"];
}
...
+(id)mountedDatabaseNodeIdentifierWithPath:(NSString*)devicePath description:(NSString*)description dictionary:(NSDictionary*)dictionary type:(NSInteger)type
{
    ...
    // does it contain an Unsething Data folder?
    BOOL isDir;
