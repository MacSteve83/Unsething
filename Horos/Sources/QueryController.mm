...
static BOOL QueryAddressIsLoopback(NSString *address)
{
	if( [address isKindOfClass: [NSString class]] == NO)
		return NO;
	
	NSString *cleanAddress = [[address stringByTrimmingCharactersInSet: [NSCharacterSet whitespaceAndNewlineCharacterSet]] lowercaseString];
	
	return [cleanAddress hasPrefix: @"127."] || [cleanAddress isEqualToString: @"localhost"] || [cleanAddress isEqualToString: @"::1"] || [cleanAddress hasPrefix: @"[::1]"];
}

static NSString *QueryPreferredAddressForServer(NSDictionary *server)
{
	NSString *address = [server valueForKey: @"Address"];
	NSString *localizedAddress = [server valueForKey: @"Indirizzo"];
	
	if( QueryAddressIsLoopback( address) && [localizedAddress isKindOfClass: [NSString class]] && QueryAddressIsLoopback( localizedAddress) == NO && [localizedAddress length] > 0)
		return localizedAddress;
	
	return address;
}

static BOOL QueryServerHasLoopbackAddress(NSDictionary *server)
{
	NSString *address = QueryPreferredAddressForServer(server);
	
	if( QueryAddressIsLoopback( address))
		return YES;
	
	NSString *addressAndPort = [server valueForKey: @"AddressAndPort"];
	
	if( [addressAndPort isKindOfClass: [NSString class]])
	{
		if( QueryAddressIsLoopback( addressAndPort))
			return YES;
		
		NSArray *components = [addressAndPort componentsSeparatedByString: @":"];
		
		if( [components count] > 0 && QueryAddressIsLoopback( [components objectAtIndex: 0]))
			return YES;
	}
...
{
	NSDictionary *sameAETitleServer = nil;
	NSUInteger sameAETitleCount = 0;
	
	for( NSUInteger i = 0 ; i < [servers count]; i++)
	{
		if( QueryServerHasLoopbackAddress( [servers objectAtIndex:i]))
			continue;
		if( [[savedServer objectForKey:@"AETitle"] isEqualToString: [[servers objectAtIndex:i] objectForKey:@"AETitle"]])
		{
			sameAETitleServer = [servers objectAtIndex:i];
			sameAETitleCount++;
			
			if( [[savedServer objectForKey:@"AddressAndPort"] isEqualToString: [NSString stringWithFormat:@"%@:%@", QueryPreferredAddressForServer([servers objectAtIndex:i]), [[servers objectAtIndex:i] valueForKey:@"Port"]]])
			{
	...
	if( sameAETitleCount == 1)
		return sameAETitleServer;
...
- (void) refreshSources
{
	...
	for( NSInteger i = (NSInteger) [serversArray count] - 1; i >= 0; i--)
		if( QueryServerHasLoopbackAddress( [serversArray objectAtIndex: i]))
	...
	for( NSUInteger i = 0; i < [savedArray count]; i++)
	{
		...
		{
			[sourcesArray addObject: [NSMutableDictionary dictionaryWithObjectsAndKeys:[[savedArray objectAtIndex: i] valueForKey:@"activated"], @"activated", [server valueForKey:@"Description"], @"name", [server valueForKey:@"AETitle"], @"AETitle", [NSString stringWithFormat:@"%@:%@", QueryPreferredAddressForServer(server), [server valueForKey:@"Port"]], @"AddressAndPort", server, @"server", nil]];
	...
	for( NSUInteger i = 0; i < [serversArray count]; i++)
	{
		...
		if( ([[server valueForKey:@"QR"] boolValue] == YES || [server valueForKey:@"QR"] == nil ))
			...
			[sourcesArray addObject: [NSMutableDictionary dictionaryWithObjectsAndKeys:[NSNumber numberWithBool: NO], @"activated", [server valueForKey:@"Description"], @"name", [server valueForKey:@"AETitle"], @"AETitle", [NSString stringWithFormat:@"%@:%@", QueryPreferredAddressForServer(server), [server valueForKey:@"Port"]], @"AddressAndPort", server, @"server", nil]];
	}
	...
	if( sendToPopup)
	{
		...
		for( NSUInteger i = 0; i < [serversArray count]; i++)
		{
			...
			NSString *title = [NSString stringWithFormat:@"%@ - %@/%@:%@", [server valueForKey:@"Description"], [server valueForKey:@"AETitle"], QueryPreferredAddressForServer(server), [server valueForKey:@"Port"]];
