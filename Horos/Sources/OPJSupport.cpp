...
typedef struct opj_memory_stream
{
    OPJ_BYTE *buffer;
    OPJ_SIZE_T length;
    OPJ_OFF_T offset;
...
{
    opj_memory_stream_t *memory = (opj_memory_stream_t *)p_user_data;
    OPJ_SIZE_T remaining;
    OPJ_SIZE_T bytesToRead;

    if (!memory || memory->offset < 0 || (OPJ_SIZE_T)memory->offset >= memory->length)
        return (OPJ_SIZE_T)-1;

    remaining = memory->length - (OPJ_SIZE_T)memory->offset;
    bytesToRead = p_nb_bytes < remaining ? p_nb_bytes : remaining;
    if (bytesToRead == 0)
        return (OPJ_SIZE_T)-1;

    memcpy(p_buffer, memory->buffer + memory->offset, bytesToRead);
    memory->offset += (OPJ_OFF_T)bytesToRead;
    return bytesToRead;
}
...
{
    opj_memory_stream_t *memory = (opj_memory_stream_t *)p_user_data;
    OPJ_OFF_T bytesToSkip;

    if (!memory || p_nb_bytes < 0 || memory->offset < 0)
        return (OPJ_OFF_T)-1;

    bytesToSkip = p_nb_bytes;
    if ((OPJ_SIZE_T)(memory->offset + bytesToSkip) > memory->length)
        bytesToSkip = (OPJ_OFF_T)(memory->length - (OPJ_SIZE_T)memory->offset);

    memory->offset += bytesToSkip;
    return bytesToSkip;
}
...
static OPJ_BOOL opj_memory_stream_seek(OPJ_OFF_T p_nb_bytes, void *p_user_data)
{
    ...
    if (!memory || p_nb_bytes < 0 || (OPJ_SIZE_T)p_nb_bytes > memory->length)
        return OPJ_FALSE;

    memory->offset = p_nb_bytes;
    return OPJ_TRUE;
}
...
static opj_stream_t *opj_create_memory_stream(OPJ_BYTE *buffer, OPJ_SIZE_T length)
{
    opj_stream_t *stream = opj_stream_create(OPJ_J2K_STREAM_CHUNK_SIZE, OPJ_TRUE);
    opj_memory_stream_t *memory;

    if (!stream)
        return NULL;

    memory = (opj_memory_stream_t *)calloc(1, sizeof(opj_memory_stream_t));
    if (!memory)
    {
    ...
    opj_stream_set_user_data(stream, memory, opj_memory_stream_free);
                                             ...
                                             int *colorModel)
{
    ...
    decodeInfo.stream = opj_create_memory_stream((OPJ_BYTE *)jp2Data, (OPJ_SIZE_T)jp2DataSize);
