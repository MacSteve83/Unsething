...
- (NSArray*) referencedObjects
{
	...
	if (status.good())
	{
        ...
        do { 
            DSRContentItem &item = doc->getTree().getCurrentContentItem(); 
            if (item.getValueType() == DSRTypes::VT_Image)
			{
				//image node get SOPCInstance
				const DSRImageReferenceValue &imageReference = item.getImageReference();
				OFString sopInstance = imageReference.getSOPInstanceUID();
