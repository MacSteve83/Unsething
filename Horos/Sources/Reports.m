...
+(void)checkForWordTemplates
{
    ...
    @try {
        ...
        // previously, we had a single word template in the Unsething Data folder
...
+ (void)checkForPagesTemplate;
{
	...
	if ([[NSFileManager defaultManager] fileExistsAtPath: defaultReport] == NO)
        [[NSFileManager defaultManager] copyItemAtPath:[[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:@"/Unsething Report.pages"] toPath:defaultReport error:NULL];
...
{	
    ...
    if( [[NSFileManager defaultManager] fileExistsAtPath:indexFilePath] == NO)
    {
        ...
        */
        ...
        NSString* path = [[NSBundle mainBundle] pathForResource:@"Unsething Report.pages09" ofType:@"pages"];
