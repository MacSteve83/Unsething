...
- (long) fixedHeight
{
    return fixedHeight + UnsethingToolbarPanelHeightExtra();
}
...
- (long) exposedHeight {
	return [self fixedHeight] - [ToolbarPanelController hiddenHeight];
}

+ (long) exposedHeight {
	return fixedHeight + UnsethingToolbarPanelHeightExtra() - [ToolbarPanelController hiddenHeight];
}
...
- (id)initForViewer:(ViewerController *)v withToolbar:(NSToolbar *)t
{
	if (self = [super initWithWindowNibName:@"ToolbarPanel"])
	{
        ...
        [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(applicationDidChangeScreenParameters:) name:UnsethingToolbarSizeDidChangeNotification object:nil];
