...
- (NSURL *)URLForSplashPage:(NSString *)page
{
    NSBundle *bundle = [NSBundle mainBundle];
    NSString *language = [[NSBundle preferredLocalizationsFromArray:@[@"en", @"it"]
                                                    forPreferences:bundle.preferredLocalizations] firstObject];
    NSString *filename = [language isEqualToString:@"it"] ? page : [page stringByAppendingString:@"-en"];
    NSURL *directory = [bundle.resourceURL URLByAppendingPathComponent:@"Splash" isDirectory:YES];
    return [directory URLByAppendingPathComponent:[filename stringByAppendingPathExtension:@"html"]];
}

- (void) awakeFromNib
{
    [[aboutWebView mainFrame] loadRequest:[NSURLRequest requestWithURL:[self URLForSplashPage:@"about"]]];
    [[releaseNotesWebView mainFrame] loadRequest:[NSURLRequest requestWithURL:[self URLForSplashPage:@"releasenotes"]]];
    [[partnersWebView mainFrame] loadRequest:[NSURLRequest requestWithURL:[self URLForSplashPage:@"partners"]]];
