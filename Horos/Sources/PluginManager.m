...
{
    NSString *name = [path lastPathComponent];
    NSString *pluginName = [[name lastPathComponent] stringByDeletingPathExtension];
    
    if ([pluginName caseInsensitiveCompare:@"HorosCloud"] == NSOrderedSame)
    {
        NSLog(@"Skipping HorosCloud plugin for Unsething");
        return;
    }
    
    if ([pluginsNames valueForKey: pluginName])
    {
    ...
    if ( [[name pathExtension] isEqualToString:@"horosplugin"] || [[name pathExtension] isEqualToString:@"unsethingplugin"] )
    {
...
+ (void) deployHorosCloudPluginAtPath:(NSString*) path deployedPlugins:(NSMutableArray*) deployedPlugins
{
    ...
    {
        NSString* pluginPath = [deployedPlugins objectAtIndex:i];
        NSBundle* bundle = [NSBundle bundleWithPath:pluginPath];
        NSString* name = [bundle.infoDictionary objectForKey:@"CFBundleName"];
        if (!name)
        {
            name = [[pluginPath lastPathComponent] stringByDeletingPathExtension];
        }

        if ([name caseInsensitiveCompare:@"HorosCloud"] == NSOrderedSame)
            [deployedPlugins removeObjectAtIndex:i];
    }

    [[NSFileManager defaultManager] removeItemAtPath:[path stringByAppendingPathComponent:@"HorosCloud.horosplugin"] error:nil];
    [[NSFileManager defaultManager] removeItemAtPath:[path stringByAppendingPathComponent:@"HorosCloud.horosplugin.zip"] error:nil];
}
...
//		BOOL allUsers = ([path isEqualToString:sysActivePath] || [path isEqualToString:sysInactivePath]);
        ...
        while(name = [e nextObject])
		{
			if(/* [[name pathExtension] isEqualToString:@"plugin"] || */ [[name pathExtension] isEqualToString:@"horosplugin"] || [[name pathExtension] isEqualToString:@"unsethingplugin"] || [[name pathExtension] isEqualToString:@"osirixplugin"])
			{
	...
	NSAutoreleasePool   *pool = [[NSAutoreleasePool alloc] init];
		...
		if (NSOKButton == button)
		{
			...
			if(pluginManagerController)
			{
                ...
                if ( ([pluginURL containsString:@"horosplugin"] || [pluginURL containsString:@"unsethingplugin"]) )
                {
