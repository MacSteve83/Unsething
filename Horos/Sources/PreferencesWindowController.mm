...
-(void)awakeFromNib
{
	...
	name = NSLocalizedString(@"Base", @"Section in preferences window");
	...
	[self addPaneWithResourceNamed:@"OSIViewerPreferencePanePref" inBundle:bundle withTitle:NSLocalizedString(@"Viewer", @"Panel in preferences window") image:[NSImage imageNamed:@"AxialSmall"] toGroupWithName:name];
	...
	[self addPaneWithResourceNamed:@"OSIListenerPreferencePanePref" inBundle:bundle withTitle:NSLocalizedString(@"Receiving", @"Panel in preferences window") image:[NSImage imageNamed:@"Network"] toGroupWithName:name];
	[self addPaneWithResourceNamed:@"OSILocationsPreferencePanePref" inBundle:bundle withTitle:NSLocalizedString(@"DICOM Nodes", @"Panel in preferences window") image:[NSImage imageNamed:@"AccountPreferences"] toGroupWithName:name];
    ...
    [self addPaneWithResourceNamed:@"BackupPreferencePane" inBundle:bundle withTitle:NSLocalizedString(@"Backup", @"Panel in preferences window") image:[NSImage imageNamed:@"BackupPreferences"] toGroupWithName:name];
...
-(void)setCurrentContext:(PreferencesWindowContext*)context
{
    ...
    {
        NSPreferencePane *targetPane = context ? context.pane : nil;
        
        if (context && !targetPane)
        {
            NSLog(@"Warning: preferences pane %@ could not be loaded", context.resourceName);
            return;
        }
        
        if (context && !targetPane.mainView)
        {
			@try
            {
				[targetPane loadMainView];
			}
            @catch (NSException* e)
            {
				NSLog(@"Warning: %@", e.description);
        ...
        if (context && !targetPane.mainView)
        {
            NSLog(@"Warning: preferences pane %@ has no main view", context.resourceName);
            return;
        }
        ...
        NSString* title = NSLocalizedString(@"Preferenze Unsething", NULL);
        NSSize newSize;
        ...
        if (!context) {
            ...
            NSView *cview = targetPane.mainView;
