...
- (IBAction) UnsethingDBsaveAs:(id)sender;
{
    [self OsiriXDBsaveAs: sender];
}

- (IBAction) refreshNodesUnsethingDB:(id)sender;
{
    [self refreshNodesOsiriXDB: sender];
}

- (IBAction) UnsethingDBloadFrom:(id)sender;
{
    [self OsiriXDBloadFrom: sender];
}
...
- (IBAction) addPath:(id) sender
{
    ...
    [oPanel beginWithCompletionHandler:^(NSInteger result) {
		...
		if( [[location lastPathComponent] isEqualToString:@"DATABASE"] && [[[location stringByDeletingLastPathComponent] lastPathComponent] isEqualToString:OsirixDataDirName])
		{
