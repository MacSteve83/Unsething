...
static BOOL DCMIsLoopbackAddress(NSString *address)
{
...
static NSString *DCMPreferredAddressForServer(NSDictionary *server)
{
    NSString *address = [server valueForKey:@"Address"];
    NSString *localizedAddress = [server valueForKey:@"Indirizzo"];
    
    if( DCMIsLoopbackAddress(address) && [localizedAddress isKindOfClass:[NSString class]] && DCMIsLoopbackAddress(localizedAddress) == NO && [localizedAddress length] > 0)
		...
		@try
		{
            ...
            BOOL toBeSaved = NO;
	            for( int i = 0 ; i < [serversArray count] ; i++)
	            {
	                ...
	                NSString *preferredAddress = DCMPreferredAddressForServer(d);
	                
	                if( [preferredAddress isKindOfClass:[NSString class]] && [[d valueForKey:@"Address"] isEqualToString: preferredAddress] == NO)
	                {
	                    NSMutableDictionary *mdict = [NSMutableDictionary dictionaryWithDictionary: d];
	                    
	                    [mdict setObject: preferredAddress forKey: @"Address"];
            ...
            if( [[NSUserDefaults standardUserDefaults] boolForKey:@"searchDICOMBonjour"])
            {
                for( NSNetService* aServer in [[DCMNetServiceDelegate sharedNetServiceDelegate] dicomServices])
                {
                    ...
                    if( hostname && DCMIsLoopbackAddress(hostname) == NO)
                    {
